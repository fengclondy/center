 ...
 */
public class UpdateHeadAdStatusTask implements IScheduleTaskDealMulti<HTDAdvertisementDTO> {
    @Resource
    private HTDHeadAdvertiseDAO htdHeadAdvertiseDAO;

    private static final Logger logger = LoggerFactory.getLogger(UpdateHeadAdStatusTask.class);
    ...
    public boolean execute(HTDAdvertisementDTO[] tasks, String ownSign) throws Exception {
        logger.info("\n 方法:[{}],入参:[{}][{}]", "UpdateHeadAdStatusTask-execute",
        ...
        try{
            if(tasks != null && tasks.length > 0){
                ...
                for(HTDAdvertisementDTO temAdvertise : htdAdvertisementDTOList){
                    if(!nowDt.before(temAdvertise.getStart_time())
                        ...
                        old.setStatus("2");
                        old.setIs_handoff("1");
                        old.setType(temAdvertise.getType());
                        old.setSort_num(temAdvertise.getSort_num());
                        old.setModify_id(0l);
                        old.setModify_name("sys");
                        this.htdHeadAdvertiseDAO.updateByTypeAndSortNum(old);
                    }
            ...
            result = false;
            logger.error("\n 方法:[{}],异常:[{}]", "UpdateHeadAdStatusTask-execute",
                    e.getStackTrace());
        }finally {
            logger.info("\n 方法:[{}],出参:[{}]", "UpdateHeadAdStatusTask-execute",
    ...
    public List<HTDAdvertisementDTO> selectTasks(String taskParameter, String ownSign, int taskQueueNum, List<TaskItemDefine> taskQueueList, int eachFetchDataNum) throws Exception {
        logger.info("\n 方法:[{}],入参:[{}][{}][{}][{}][{}]", "UpdateHeadAdStatusTask-selectTasks",
        ...
        try{
            if (taskQueueList != null && taskQueueList.size() > 0) {
                ...
                htdAdvertisementDTOList = htdHeadAdvertiseDAO.queryHeadAdList4Task(condition,pager);
            }
        }catch (Exception e){
            logger.error("\n 方法:[{}],异常:[{}]", "UpdateHeadAdStatusTask-selectTasks",
                    e.getStackTrace());
        }finally {
            logger.info("\n 方法:[{}],出参:[{}]", "UpdateHeadAdStatusTask-selectTasks",
    ...
    public void updateStatus(HTDAdvertisementDTO htdAdvertisementDTO){
        ...
        this.htdHeadAdvertiseDAO.updateHeadAdStatusById(historyDTO);
