 ...
 */
public class UpdateStaticAdvertiseStatusTask implements IScheduleTaskDealMulti<HTDAdvertisementDTO> {
    @Resource
    private HTDStaticAdvertiseDAO htdStaticAdvertiseDAO;

    private static final Logger logger = LoggerFactory.getLogger(UpdateStaticAdvertiseStatusTask.class);
    ...
    public boolean execute(HTDAdvertisementDTO[] tasks, String ownSign) throws Exception {
        logger.info("\n 方法:[{}],入参:[{}][{}]", "UpdateStaticAdvertiseStatusTask-execute",
        ...
        try{
            if(tasks != null && tasks.length > 0){
                ...
                for(HTDAdvertisementDTO temAdvertise : htdAdvertisementDTOList){
                    if(!nowDt.before(temAdvertise.getStart_time())
                        ...
                        this.htdStaticAdvertiseDAO.updateBySortNum(old);
                    }
            ...
            result = false;
            logger.error("\n 方法:[{}],异常:[{}]", "UpdateStaticAdvertiseStatusTask-execute",
                    e.getStackTrace());
        }finally {
            logger.info("\n 方法:[{}],出参:[{}]", "UpdateStaticAdvertiseStatusTask-execute",
    ...
    public List<HTDAdvertisementDTO> selectTasks(String taskParameter, String ownSign, int taskQueueNum, List<TaskItemDefine> taskQueueList, int eachFetchDataNum) throws Exception {
        logger.info("\n 方法:[{}],入参:[{}][{}][{}][{}][{}]", "UpdateStaticAdvertiseStatusTask-selectTasks",
        ...
        try{
            if (taskQueueList != null && taskQueueList.size() > 0) {
                ...
                htdAdvertisementDTOList = htdStaticAdvertiseDAO.queryStaticAdList4Task(condition,pager);
            }
        }catch (Exception e){
            logger.error("\n 方法:[{}],异常:[{}]", "UpdateStaticAdvertiseStatusTask-selectTasks",
                    e.getStackTrace());
        }finally {
            logger.info("\n 方法:[{}],出参:[{}]", "UpdateStaticAdvertiseStatusTask-selectTasks",
    ...
    public void updateStatus(HTDAdvertisementDTO htdAdvertisementDTO){
        ...
        this.htdStaticAdvertiseDAO.updateStaticAdStatusById(historyDTO);
