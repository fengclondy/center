 ...
 */
public class UpdateSubCarouselAdStatusTask implements IScheduleTaskDealMulti<HTDAdvertisementDTO> {
    @Resource
    private HTDCityCarouselAdvertiseDAO htdCityCarouselAdvertiseDAO;

    private static final Logger logger = LoggerFactory.getLogger(UpdateSubCarouselAdStatusTask.class);
    ...
    public boolean execute(HTDAdvertisementDTO[] tasks, String ownSign) throws Exception {
        logger.info("\n 方法:[{}],入参:[{}][{}]", "UpdateSubCarouselAdStatusTask-execute",
        ...
        try{
            if(tasks != null && tasks.length > 0){
                ...
                for(HTDAdvertisementDTO temAdvertise : htdAdvertisementDTOList){
                    if(!nowDt.before(temAdvertise.getStart_time())
                        ...
                        old.setStatus("2");
                        old.setIs_handoff("1");
                        old.setSub_id(temAdvertise.getSub_id());
                        old.setSort_num(temAdvertise.getSort_num());
                        old.setModify_id(0l);
                        old.setModify_name("sys");
                        this.htdCityCarouselAdvertiseDAO.updateBySubIdAndSortNum(old);
                    }
            ...
            result = false;
            logger.error("\n 方法:[{}],异常:[{}]", "UpdateSubCarouselAdStatusTask-execute",
                    e.getStackTrace());
        }finally {
            logger.info("\n 方法:[{}],出参:[{}]", "UpdateSubCarouselAdStatusTask-execute",
    ...
    public List<HTDAdvertisementDTO> selectTasks(String taskParameter, String ownSign, int taskQueueNum, List<TaskItemDefine> taskQueueList, int eachFetchDataNum) throws Exception {
        logger.info("\n 方法:[{}],入参:[{}][{}][{}][{}][{}]", "UpdateSubCarouselAdStatusTask-selectTasks",
        ...
        try{
            if (taskQueueList != null && taskQueueList.size() > 0) {
                ...
                htdAdvertisementDTOList = htdCityCarouselAdvertiseDAO.querySubCarouselAdList4Task(condition,pager);
            }
        }catch (Exception e){
            logger.error("\n 方法:[{}],异常:[{}]", "UpdateSubCarouselAdStatusTask-selectTasks",
                    e.getStackTrace());
        }finally {
            logger.info("\n 方法:[{}],出参:[{}]", "UpdateSubCarouselAdStatusTask-selectTasks",
    ...
    public void updateStatus(HTDAdvertisementDTO htdAdvertisementDTO){
        ...
        this.htdCityCarouselAdvertiseDAO.updateSubCarouselAdStatusById(historyDTO);
