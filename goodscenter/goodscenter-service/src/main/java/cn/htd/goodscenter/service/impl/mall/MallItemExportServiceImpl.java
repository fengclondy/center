...
public class MallItemExportServiceImpl implements MallItemExportService {
    ...
    public ExecuteResult<MallSkuOutDTO> queryMallItemDetailPage(MallSkuInDTO mallSkuInDTO) {
        ...
        try {
            ...
            /** 只有外部供应商有SKU图片 **/
            if (ProductChannelEnum.EXTERNAL_SUPPLIER.getCode().equals(mallSkuOutDTO.getProductChannelCode())) {
                List<ItemSkuPicture> itemSkuPictureList = this.itemSkuDAO.selectSkuPictureBySkuId(mallSkuOutDTO.getSkuId());
            ...
            if (ProductChannelEnum.INTERNAL_SUPPLIER.getCode().equals(productChannelCode)) {
                if (isBoxFlag != null) {
                    if (itemStatus.equals(HtdItemStatusEnum.SHELVED.getCode())) {
                        ...
                        String shelfType = isBoxFlag.equals(Constants.BOX_FLAG_IS_BOX) ? Constants.SHELF_TYPE_IS_BOX : Constants.SHELF_TYPE_IS_AREA;
                        ItemSkuPublishInfo itemSkuPublishInfo1 = this.itemSkuPublishInfoMapper.selectByItemSkuAndShelfType(skuId, shelfType, Constants.IS_VISABLE_TRUE);
     ...
     */
    @Override
    public ExecuteResult<MallSkuWithStockOutDTO> queryMallItemDetailWithStock(MallSkuWithStockInDTO mallSkuWithStockInDTO) {
        ...
        try {
            ...
            if (mallSkuOutDTOExecuteResult.isSuccess()) {
                ...
                ExecuteResult<MallSkuStockOutDTO> mallSkuStockOutDTOExecuteResult = this.queryMallItemStockInfo(mallSkuStockInDTO);
     ...
     */
    @Override
    public ExecuteResult<List<MallSkuWithStockOutDTO>> queryMallItemDetailWithStockList(List<MallSkuWithStockInDTO> mallSkuWithStockInDTOList) {
     ...
     */
    @Override
    public ExecuteResult<MallChoiceBoxProductOutDTO> choiceMallItemBoxFlagAndStockInfo(final MallChoiceBoxProductInDTO mallSkuStockInDTO) {
        // 校验入参
...
//            });
            ...
            if (ProductChannelEnum.INTERNAL_SUPPLIER.getCode().equals(productChannelCode)) {
                ...
                if (itemStatus.equals(HtdItemStatusEnum.SHELVED.getCode())) { // 0. item是上架状态
                    // 首先查询商品是否在包厢或者大厅上架
                    boolean isBoxProduct = false; // 包厢是否上架
                    ItemSkuPublishInfo boxItemSkuPublishInfo = this.itemSkuPublishInfoMapper.selectByItemSkuAndShelfType(skuId, "1", "1");
                    ...
                    ItemSkuPublishInfo areaItemSkuPublishInfo = this.itemSkuPublishInfoMapper.selectByItemSkuAndShelfType(skuId, "2", "1");
                    ...
                    if (isLogin == 0) { // 1.1 未登录
                        if (isBoxProduct && isAreaProduct) { // 1.11 未登录 如果区域和包厢都上架，优先选择大厅
                            isUpShelf = 1;
...
//        }
        ...
        try {
            ...
            // 预售
            mallSkuOutDTO.setIsPreSale(item.isPreSale() ? 1 : 0);
            if (item.isPreSale()) {
...
//            redisDB.setAndExpire(key, redisValue, 300);
    ...
    /**
     * 查询商品品牌品类详细信息
     * @param mallSkuOutDTO 目标
     */
    private void setCategoryAndBrandInfo(MallSkuOutDTO mallSkuOutDTO) {
        /** 品类信息 **/
        ExecuteResult<Map<String, Object>> executeResultCategory = this.itemCategoryService.queryItemOneTwoThreeCategoryName(mallSkuOutDTO.getThirdCategoryId(), ">>");
    ...
    private String queryDefaultSkuCodeByItemCode(String itemCode) throws Exception {
        ...
        try {
            ...
            if (ProductChannelEnum.INTERNAL_SUPPLIER.getCode().equals(productChannelCode) || ProductChannelEnum.JD_PRODUCT.getCode().equals(productChannelCode)) { // 内部供应商 和京东商品
                ...
                if (skuList.size() == 1) { // item 和 item_sku 1v1
                    ...
                    throw new Exception("内部供应商商品和京东商品根据itemCode期望查到一条SKU, 实际查询到的条数 : " + skuList.size() + ", itemCode : " + itemCode);
                }
                ...
                if (skuList.size() > 0) {
                    for (ItemSku itemSku : skuList) {
                        ...
                        if (itemSku.getSkuType().equals(1)) {
                            ItemSkuPublishInfo itemSkuPublishInfo = this.itemSkuPublishInfoMapper.selectByItemSkuAndShelfType(itemSku.getSkuId(), "2", "1");
                ...
                throw new Exception("商品渠道编码非法, productChannelCode : " + productChannelCode + ", itemCode : " + itemCode);
            }
        } catch (Exception e) {
                    ...
                    if (attributeValueObj instanceof JSONArray) {
                        ...
                        for (int i = 0; i < size; i++) {
                            ...
                            if (!StringUtils.isEmpty(attributeValueName)) {
                                MallSkuAttributeDTO mallSkuAttributeDTO = getMallSkuAttributeDTO(attributeId, attributeName, attributeValueId, attributeValueName);
    ...
    private MallSkuAttributeDTO getMallSkuAttributeDTO(Long attributeId, String attributeName, Integer attributeValueId, String attributeValueName) {
    ...
    public ExecuteResult<List<NewPublishItemOutDTO>> queryNewPublishItemList(
        ...
        if("1".equals(queryNewPublishItemInDTO.getIsLoginFlag())&&!queryNewPublishItemInDTO.isBelongsHtd()){//已经登陆，则优先商品＋
             ...
             ExecuteResult<DataGrid<ProductPlusItemDTO>> productPlusList=productPlusExportService.querySellerProductPlusProductList(productPlusItemDTO,pager);
             if(productPlusList.isSuccess()&&productPlusList.getResult()!=null&&CollectionUtils.isNotEmpty(productPlusList.getResult().getRows())){
                 List<ProductPlusItemDTO> rows=productPlusList.getResult().getRows();
                 for(ProductPlusItemDTO itemPlusItem:rows){
                     if(StringUtils.isNotEmpty(queryNewPublishItemInDTO.getItemCode())&&queryNewPublishItemInDTO.getItemCode().equals(itemPlusItem.getItemCode())){
                         continue;
                     }
                     Item t=new Item();
        ...
        if(itemList.size()<queryNewPublishItemInDTO.getCount()*2){
            ...
            if ("0".equals(queryNewPublishItemInDTO.getSupplierType())){
            	...
            	List<AddressInfo> childAddressList=addressUtil.getAddressList(queryNewPublishItemInDTO.getCurrentSiteCode());
    ...
    @Override
    public ExecuteResult<List<MallSearchItemDTO>> queryMallSearchItemInfo(
    	...
    	if(CollectionUtils.isNotEmpty(searchItemList)){
    		for(MallSearchItemDTO mallSearchItemDTO:searchItemList){
    			...
    			if(ProductChannelEnum.EXTERNAL_SUPPLIER.getCode().equals(mallSearchItemDTO.getItemChannelCode())){
    				ExecuteResult<BigDecimal> priceResult=itemSkuPriceService.queryMinLadderPriceByItemId(mallSearchItemDTO.getItemId());
    				...
    				if(CollectionUtils.isNotEmpty(skuList)&&skuList.get(0)!=null){
    					//查询价格
            			ExecuteResult<ItemSkuBasePriceDTO> priceResult=itemSkuPriceService.queryItemSkuBasePrice(skuList.get(0).getSkuId());
            			if(priceResult.isSuccess()&&priceResult.getResult()!=null){
    ...
    @Override
    public ExecuteResult<ItemSalesAreaDTO> queryInnerSupplierItemSalesArea(
            Long itemId, Integer isBoxFlag) {
        ...
        List<ItemSalesAreaDetail> salesAreaDetailList=
                itemSalesAreaDetailMapper.selectAreaDetailsBySalesAreaId(itemSalesArea.getSalesAreaId());
    ...
    @Override
    public ExecuteResult<List<HotSellItemOutDTO>> queryHotSellItemList(
            QueryHotSellItemInDTO queryHotSellItemInDTO) {
        ...
        if("1".equals(queryHotSellItemInDTO.getIsLoginFlag())&&!queryHotSellItemInDTO.isBelongsHtd()){
             ...
             SellerOuterProductChannel sellerOuterProductChannelDb = this.sellerOuterProductChannelMapper.selectBySellerIdAndChannelCode(sellerOuterProductChannel);

             if (sellerOuterProductChannelDb != null && sellerOuterProductChannelDb.getAccessStatus() ==1) {
                 //优先查询热销商品+商品
                 List<HotSellItemOutDTO> prodPlusHotSellList=itemMybatisDAO.queryProductPlusItemList(queryHotSellItemInDTO);
        ...
        for(HotSellItemOutDTO hotSellItemOutDTO:hotSellItemList){
            //List<ItemPicture> itemPicList=itemPictureDAO.queryItemPicsById(hotSellItemOutDTO.getItemId());
    ...
    public boolean isInSaleArea(Long itemId, Integer isBoxFlag, String cityCode) {
    	if("1".equals(cityCode)){
          ...
          if(currentAddress != null){
            ...
            int count = this.itemMybatisDAO.queryItemIsInSaleArea(itemId, isBoxFlag, parentCityCode, cityCode, childCodeList);
            return count >= 1;
    }
    ...
    public boolean isBoxProduct(Long skuId, String cityCode) {
        boolean isBoxProduct = false; // 包厢是否上架
        ItemSkuPublishInfo boxItemSkuPublishInfo = this.itemSkuPublishInfoMapper.selectByItemSkuAndShelfType(skuId, Constants.SHELF_TYPE_IS_BOX, Constants.IS_VISABLE_TRUE);
        if (boxItemSkuPublishInfo != null) { //  包厢上架
            ItemSku itemSku = this.itemSkuDAO.queryItemSkuBySkuId(skuId);
    ...
    public boolean isAreaProduct(Long skuId, String cityCode) {
        ...
        ItemSkuPublishInfo boxItemSkuPublishInfo = this.itemSkuPublishInfoMapper.selectByItemSkuAndShelfType(skuId, Constants.SHELF_TYPE_IS_AREA, Constants.IS_VISABLE_TRUE);
        if (boxItemSkuPublishInfo != null) { //  包厢上架
            ItemSku itemSku = this.itemSkuDAO.queryItemSkuBySkuId(skuId);
     ...
     */
    @Override
    public ExecuteResult<DataGrid<MallRecommendItemOutDTO>> queryRecommendItemList(MallRecommendItemInDTO mallRecommendItemInDTO, Pager pager) {
        ...
        try {
            // 参数校验
            if (mallRecommendItemInDTO == null || StringUtils.isEmpty(mallRecommendItemInDTO.getBuyerCode()) || mallRecommendItemInDTO.getBuyerId() == null
                    || StringUtils.isEmpty(mallRecommendItemInDTO.getAreaCode()) || mallRecommendItemInDTO.getBoxSellerDtoList() == null
                    || mallRecommendItemInDTO.getPromotionItemIdList() == null || pager == null) {
                logger.info("查询推荐商品校验不通过，错误信息：入参都不能为空, mallRecommendItemInDTO: {}, pager : {}", JSON.toJSONString(mallRecommendItemInDTO), JSON.toJSONString(pager));
                executeResult.setCode(ResultCodeEnum.INPUT_PARAM_IS_NULL.getCode());
                executeResult.setResultMessage(ResultCodeEnum.INPUT_PARAM_IS_NULL.getMessage());
            ...
            List<MallRecommendItemOutDTO> oneItem4EveryBoxSellerDTOList = this.queryOneItem4AllBoxSeller(mallRecommendItemInDTO);
//            // 排除直营和查询大B满足条件的一款商品相同的商品
//            if (htdMallRecommendItem != null && oneItem4EveryBoxSellerDTOList != null) {
//                for (MallRecommendItemOutDTO mallRecommendItemOutDTO : oneItem4EveryBoxSellerDTOList) {
//                    if (mallRecommendItemOutDTO.getItemCode().equals(htdMallRecommendItem.getItemCode())) {
...
//            }
            ...
            // 返回分页结果
            List<MallRecommendItemOutDTO> pagedItemOutDTOList = this.getLimitBoxSellerIdList(mallRecommendItemOutDTOList, pager.getPageOffset(), pager.getRows() + pager.getPageOffset());
     ...
     * @return
     */
    private MallRecommendItemOutDTO getOneRecommendItem(MallRecommendItemInDTO mallRecommendItemInDTO) throws Exception {
     ...
     * @return
     */
    private List<MallRecommendItemOutDTO> queryOneItem4AllBoxSeller(MallRecommendItemInDTO mallRecommendItemInDTO) throws Exception {
        List<MallRecommendItemOutDTO> mallRecommendItemOutDTOList = new ArrayList<>();
        ...
        OrderRecentQueryPurchaseRecordsOutDTO queryPurchaseRecordsOutDTO = orderQueryAPI.queryPurchaseRecordsByBuyerCodeAndSellerCode(orderRecentQueryPurchaseRecordsInDTO);
        ...
        if (ResultCodeEnum.SUCCESS.getCode().equals(queryPurchaseRecordsOutDTO.getResponseCode())) {
            // 购买记录list
            List<OrderRecentQueryPurchaseRecordOutDTO> orderRecentQueryPurchaseRecordOutDTOList = queryPurchaseRecordsOutDTO.getOrderRecentQueryPurchaseRecordOutDTOList();
            // 购买记录是否大于0
            if (orderRecentQueryPurchaseRecordOutDTOList != null && orderRecentQueryPurchaseRecordOutDTOList.size() > 0) {
                // 批量检查这些item是否满足可买条件
                List<String> itemCodeList = this.getItemCodeListFromRecentQueryPurchaseRecord(orderRecentQueryPurchaseRecordOutDTOList);
                List<NameValuePair> params = new ArrayList<>();
                params.add(new NameValuePair("buyerId", String.valueOf(buyerId)));
                params.add(new NameValuePair("addressCode", String.valueOf(areaCode)));
                params.add(new NameValuePair("itemCodeList", JSON.toJSONString(itemCodeList)));
                ...
                if (itemSearchResultDTO != null && itemSearchResultDTO.getStatus().equals("success")) {
                    ...
                    for (OrderRecentQueryPurchaseRecordOutDTO orderRecentQueryPurchaseRecordOutDTO : orderRecentQueryPurchaseRecordOutDTOList) {
                        ...
                        for (ItemSearchItemDataDTO itemSearchItemDataDTO : itemDateList) {
                            ...
                            if (orderRecentQueryPurchaseRecordOutDTO.getItemCode().equals(itemSearchItemDataDTO.getItemCode())) {
        ...
        List<String> needSearchSolrSellerIdList = this.getNeedSearchSolrSellerIdList(boxSellerIdList, hasPurchaseRecordSellerIdList);
        if (needSearchSolrSellerIdList.size() > 0) {
            mallRecommendItemOutDTOList.addAll(this.queryNewestItemBySearchor(buyerId, needSearchSolrSellerIdList, areaCode, promotionIdList));
        }
        return mallRecommendItemOutDTOList;
    }
     ...
     * @return
     */
    private List<String> getItemCodeListFromRecentQueryPurchaseRecord(List<OrderRecentQueryPurchaseRecordOutDTO> orderRecentQueryPurchaseRecordOutDTOList) {
        List<String> itemCodeList = new ArrayList<String>();
        for (OrderRecentQueryPurchaseRecordOutDTO orderRecentQueryPurchaseRecordOutDTO : orderRecentQueryPurchaseRecordOutDTOList) {
     ...
     * @return
     */
    private List<MallRecommendItemOutDTO> queryNewestItemBySearchor(Long buyerId, List<String> needSearchSolrSellerIdList, String areaCode, List<Long> promotionIdList) {
        List<MallRecommendItemOutDTO> mallRecommendItemOutDTOList = new ArrayList<>();
        try {
            List<NameValuePair> params = new ArrayList<>();
            params.add(new NameValuePair("buyerId", String.valueOf(buyerId)));
     ...
     * @return
     */
    private static List<MallRecommendItemOutDTO> getLimitBoxSellerIdList(List<MallRecommendItemOutDTO> mallRecommendItemOutDTOList, int startIndex, int endIndex) {
     ...
     * @return
     */
    private List<String> getNeedSearchSolrSellerIdList(List<String> boxSellerIdList, List<String> hasPurchaseRecordSellerIdList) {
