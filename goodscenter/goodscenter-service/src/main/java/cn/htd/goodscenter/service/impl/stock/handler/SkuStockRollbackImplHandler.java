...
public class SkuStockRollbackImplHandler extends AbstractSkuStockChangeHandler {
    ...
    protected void doChange(Order4StockEntryDTO order4StockEntryDTO, Long stockId, boolean isSpecialOrder) throws Exception {
        ...
        // 幂等查询
        if(idempotentHandle(orderNo, stockId, StockTypeEnum.ROLLBACK, messageId, isSpecialOrder)) {
            return;
        }
        // ADD-START 校验该订单的商品库存有没有扣减过
        if (!validateStockChangePreCondition(orderNo, stockId, isSpecialOrder, StockTypeEnum.REDUCE)) {
            // 如果没有校验通过
