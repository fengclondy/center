...
public class B2cCouponInfoSyncServiceImpl implements B2cCouponInfoSyncService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(B2cCouponInfoSyncServiceImpl.class);
    ...
    public ExecuteResult<String> saveB2cCouponInfoSync(B2cCouponInfoSyncDTO b2cCouponInfoSyncDTO) {
        ...
        try {
            ...
            // 输入DTO的验证
            ValidateResult validateResult = ValidationUtils.validateEntity(b2cCouponInfoSyncDTO);
            // 有错误信息时返回错误信息
            ...
            if (null == discountPercent || discountPercent.intValue() < 0 || discountPercent.intValue() > 100) {
                throw new MarketCenterBusinessException(MarketCenterCodeConst.PARAMETER_ERROR,
                        "折扣券单次使用百分比值不满足规则:" + discountPercent);
            }
            Date date = new Date();
            if (b2cCouponInfoSyncDTO.getCouponEndTime().before(date)) {
                throw new MarketCenterBusinessException(MarketCenterCodeConst.PARAMETER_ERROR,
                        "结束时间不能小于当前时间:" + b2cCouponInfoSyncDTO.getCouponEndTime());
            }
            B2cCouponInfoSyncDMO b2cCouponInfoSyncDMO =
                    JSON.parseObject(JSON.toJSONString(b2cCouponInfoSyncDTO), B2cCouponInfoSyncDMO.class);
            // 1-校验本次请求是否和最新一次修改的内容相同
            B2cCouponInfoSyncDMO daoRes =
                    b2cCouponInfoSyncHistoryDAO.queryB2cCouponInfoSyncHistory(b2cCouponInfoSyncDMO);
            if (null == daoRes || (!b2cCouponInfoSyncDTO.getCouponName().equals(daoRes.getCouponName())
                    ...
                    || !b2cCouponInfoSyncDTO.getCouponType().equals(daoRes.getCouponType()) || !b2cCouponInfoSyncDTO
                    .getCouponProvideType().equals(daoRes.getCouponProvideType())
                ...
                marketRedisDB.addSet(RedisConst.REDIS_SYNC_B2C_COUPON_SET, b2cCouponInfoSyncDTO.getB2cActivityCode());
            }
