...
public class B2cCouponUseLogSyncServiceImpl implements B2cCouponUseLogSyncService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(B2cCouponUseLogSyncServiceImpl.class);
    ...
    public ExecuteResult<String> saveB2cCouponUseLogSync(B2cCouponUseLogSyncDTO b2cCouponUseLogSyncDTO) {
        ...
        try {
            ...
            // 输入DTO的验证
            ValidateResult validateResult = ValidationUtils.validateEntity(b2cCouponUseLogSyncDTO);
            // 有错误信息时返回错误信息
            ...
            if (StringUtils.isEmpty(memberId) && StringUtils.isEmpty(memberCode)) {
                throw new MarketCenterBusinessException(MarketCenterCodeConst.PARAMETER_ERROR,
                        "参数memberId:" + memberId + "和参数memberCode:" + memberCode + "不能同时为空");
            }
            String b2cSellerCode = "";
            if (!StringUtils.isEmpty(memberCode)) {
                b2cSellerCode = memberCode;
            } else {
                ExecuteResult<String> memRaoRes = memberBaseInfoService.getMemberCodeById(Long.parseLong(memberId));
            ...
            B2cCouponUseLogSyncDMO b2cCouponUseLogSyncDMO =
                    JSON.parseObject(JSON.toJSONString(b2cCouponUseLogSyncDTO), B2cCouponUseLogSyncDMO.class);
            ...
            result.setCode(MarketCenterCodeConst.RETURN_SUCCESS);
            logger.error("插入数据库是违反了主键唯一约束  请求参数:{}", JSON.toJSONString(b2cCouponUseLogSyncDTO));
