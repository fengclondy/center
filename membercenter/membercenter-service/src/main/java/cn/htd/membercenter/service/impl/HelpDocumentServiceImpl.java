...
import cn.htd.membercenter.dao.HelpDocCategoryDAO;
import cn.htd.membercenter.dao.HelpDocTopicDAO;
import cn.htd.membercenter.domain.HelpDocCategory;
import cn.htd.membercenter.domain.HelpDocFirstCategory;
import cn.htd.membercenter.domain.HelpDocSecondCategory;
...
import cn.htd.membercenter.service.HelpDocumentService;
...
//			helpDocTopicDTO.setCategoryCode(categoryCode);
			...
			List<HelpDocTopic> helpDocTopicList = helpDocTopicDAO.getHelpDocTopicsByParams(helpDocTopicDTO);
	...
	@Override
	public ExecuteResult<HelpDocFirstCategory> getFirstCategoriesByFirstCategory(String firstCategory) {
		
		logger.info("HelpDocumentServiceImpl--->getFirstCategoriesByFirstCategory--->firstCategory:" + firstCategory);
		
		HelpDocFirstCategory helpDocFirstCategory = new HelpDocFirstCategory();
		
		HelpDocTopic currentHelpDocTopic = new  HelpDocTopic();
		
		ExecuteResult<HelpDocFirstCategory> result = new ExecuteResult<HelpDocFirstCategory>();
		result.setCode(HelpDocConstants.EXECUTE_RESULT_SUCCESS);
		result.setResultMessage(HelpDocConstants.EXECUTE_RESULT_SUCCESS_TEXT);
		...
		try {
			...
			//当前分类
			HelpDocCategory currentHelpDocCategory = helpDocCategoryDAO.getSingleHelpDocCategoriesByParams(helpDocCategoryDTO);
			...
			//查询二级分类
			List<HelpDocCategory> secondCategoryList = helpDocCategoryDAO.getHelpDocCategoriesByParams(helpDocCategoryDTO);
			...
			for(int i=0;i<secondCategoryList.size();i++){
				...
				List<HelpDocTopic> helpDocTopicList = helpDocTopicDAO.getNoContentFieldOfTopicsByParams(helpDocTopicDTO);
				// 设置二级分类主题
				...
				if(i==0){
					...
					if(null != helpDocTopicList && helpDocTopicList.size() > 0){
						...
						//查询当前主题
						HelpDocTopic currentTopic = helpDocTopicDAO.getHelpDocTopicsByTopicCode(currentTopicCode);
						//设置当前主题
	...
	@Override
    public ExecuteResult<HelpDocFirstCategory> getSecondCategoriesByFirstCategory(String secondCategory) {
	...
	@Override
    public ExecuteResult<HelpDocFirstCategory> getHelpDocTopicByTopictCode(String topictCode) {
		...
		try {
			//查询当前主题
			HelpDocTopic currentTopic = helpDocTopicDAO.getHelpDocTopicsByTopicCode(topictCode);
			...
			//一级分类
			HelpDocCategory firstHelpDocCategory = helpDocCategoryDAO.getSingleHelpDocCategoriesByParams(helpDocCategoryDTO);
