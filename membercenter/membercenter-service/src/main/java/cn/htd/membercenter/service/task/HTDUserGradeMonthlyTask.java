...
public class HTDUserGradeMonthlyTask implements IScheduleTaskDealMulti<MemberImportSuccInfoDTO> {

	protected static transient Logger logger = LoggerFactory.getLogger(HTDUserGradeMonthlyTask.class);
	...
	public List<MemberImportSuccInfoDTO> selectTasks(String taskParameter, String ownSign, int taskQueueNum,
		...
		logger.info("\n 方法[{}]，入参：[{}]", "HTDUserGradeMonthlyTask-selectTasks", JSONObject.toJSONString(taskParameter),
		...
		for (MemberImportSuccInfoDTO memberImportSuccInfoDTO : list) {
			logger.debug("-----------!!!!!!!!!!!!------------------------------------------------------------"
					+ memberImportSuccInfoDTO.getMemberId());
			...
			if (memberGradeModel != null) {
				memberGradeService.downgradeHTDUserGrade(memberImportSuccInfoDTO, memberGradeModel);
			}
		}
		//int onum = memberGradeDAO.getMonthCnt();
		//if (onum == 0) {
