...
public class OrderCreateAPIImpl implements OrderCreateAPI {
	...
	public OrderCreateInfoResDTO orderCreate(OrderCreateInfoReqDTO orderCreateInfoReqDTO) {
		...
		try {
			 ...
			 * validateParam(orderCreateInfoReqDTO,orderCreateInfoResDTO);
			 * if(!ResultCodeEnum.SUCCESS.getCode().equals(orderCreateInfoResDTO
			 * .getResponseCode())){ return orderCreateInfoResDTO; }
			 */
	...
	/*
	 * private OrderCreateInfoResDTO validateParam(OrderCreateInfoReqDTO
	 * orderCreateInfoReqDTO,OrderCreateInfoResDTO orderCreateInfoResDTO){
	 * 
	 * if(StringUtilHelper.isNull(orderCreateInfoReqDTO.getMessageId())){
	 * orderCreateInfoResDTO.setResponseCode(ResultCodeEnum.
	 * ORDERCENTER_PARAM_IS_NULL.getCode());
	 * orderCreateInfoResDTO.setReponseMsg(ResultCodeEnum.
	 * ORDERCENTER_PARAM_IS_NULL.getMsg()); }else
	 * if(StringUtilHelper.isNull(orderCreateInfoReqDTO.getTradeNo())){
	 * orderCreateInfoResDTO.setResponseCode(ResultCodeEnum.
	 * ORDERCENTER_CREAREORDER_PARAM_IS_NULL.getCode());
	 * orderCreateInfoResDTO.setReponseMsg(ResultCodeEnum.
	 * ORDERCENTER_CREAREORDER_PARAM_IS_NULL.getMsg()+" tradeNo:"
	 * +orderCreateInfoReqDTO.getTradeNo()); //TODO 到时候定义错误码 并set }else{
	...
	private OrderCreateInfoResDTO validateParam4MiddleWare(OrderCreateInfoReqDTO orderCreateInfoReqDTO,
		...
		if (null != orderList && orderList.size() > 0) {
			for (OrderCreateListInfoReqDTO orderTemp : orderList) {
				...
				if (!StringUtilHelper.parseNumFormat(orderNo) || orderNo.length() != 19 || !orderNo.startsWith("20")) {
					orderCreateInfoResDTO.setResponseCode(ResultCodeEnum.GOODSCENTER_PARAM_IS_ERROR.getCode());
					orderCreateInfoResDTO
							.setReponseMsg("订单号" + orderNo + ResultCodeEnum.GOODSCENTER_PARAM_IS_ERROR.getMsg());
					return orderCreateInfoResDTO;
				}
				...
				if (null != orderItemList && orderItemList.size() > 0) {
					for (OrderCreateItemListInfoReqDTO orderItemTemp : orderItemList) {
						...
						if (!StringUtilHelper.parseNumFormat(orderItemNo) || orderItemNo.length() != 21
								|| !orderItemNo.startsWith("20")) {
							orderCreateInfoResDTO.setResponseCode(ResultCodeEnum.GOODSCENTER_PARAM_IS_ERROR.getCode());
							orderCreateInfoResDTO.setReponseMsg(
									"订单行号" + orderItemNo + ResultCodeEnum.GOODSCENTER_PARAM_IS_ERROR.getMsg());
							return orderCreateInfoResDTO;
						}
					}
				} else {
	...
	public OrderCreateInfoResDTO orderCreate4Huilin(OrderCreate4huilinReqDTO orderCreate4huilinReqDTO) {
		LOGGER.info("汇掌柜请求--创建订单开始:" + JSONObject.toJSONString(orderCreate4huilinReqDTO));
		ValidateResult validateResult = DTOValidateUtil.validate(orderCreate4huilinReqDTO);
		OrderCreateInfoResDTO orderCreateInfoResDTO = new OrderCreateInfoResDTO();
		// 验证入参是否有空值
		if (!validateResult.isPass()) {
			orderCreateInfoResDTO.setReponseMsg(validateResult.getReponseMsg());
