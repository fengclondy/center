...
public class OrderCompensateERPServiceImpl implements OrderCompensateERPService{
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(OrderCompensateERPServiceImpl.class);
	...
	@Override
	public TradeOrderItemsDMO selectTradeOrderItemsInfo(
	...
	public List<TradeOrderItemsWarehouseDetailDMO> selectTradeOrderItemsWarehouseDetailList(
		...
		return tradeOrderErpDistributionDao.selectTradeOrderItemsWarehouseDetailList(tradeOrderItemsWarehouseDetailDMO);
	}
	 ...
	 */
	@Override
	public List<OrderCompensateERPDMO> selectErpDistributionOrdersList(
			Map paramMap) {
	...
	public void compensateERP(OrderCompensateERPDMO[] tasks) {
		outterLoop : for(OrderCompensateERPDMO requestInfo : tasks){
	    	...
	    	if(!StringUtilHelper.allIsNull(requestInfo.getBuyerRemarks(),requestInfo.getOrderRemarks())){	    		
	    		orderCompensateERPDTO.setOrderNote(requestInfo.getBuyerRemarks()+"|"+requestInfo.getOrderRemarks());
	    	}
	    	
	    	if(StringUtilHelper.isNotNull(orderCompensateERPDTO.getInvoiceType())){
	    		if(orderCompensateERPDTO.getInvoiceType().equals(MiddleWareEnum.MIDDLE_WARE_JD_INVOICETYPE_COMMON.getCode())){
	    			orderCompensateERPDTO.setInvoiceType(MiddleWareEnum.ERP_INVOICETYPE_COMMON.getCode());
	    		}else if(orderCompensateERPDTO.getInvoiceType().equals(MiddleWareEnum.MIDDLE_WARE_JD_INVOICETYPE_VAT.getCode())){
	    			orderCompensateERPDTO.setInvoiceType(MiddleWareEnum.ERP_INVOICETYPE_VAT.getCode());
	    		}
	    	}
	    	...
	    	if(StringUtilHelper.isNotNull(requestInfo.getOrderItemNos(),requestInfo.getOrderNo())){
	    		if(StringUtilHelper.isNotNull(requestInfo.getOrderFrom()) 
	    				&& !requestInfo.getOrderFrom().equals(OrderStatusEnum.ORDER_FROM_VMS.getCode())){	    			
	    			...
	    			if(null != payOrderInfoDMO && StringUtilHelper.isNotNull(payOrderInfoDMO.getPayType()) && payOrderInfoDMO.getPayType().toString().equals(PayStatusEnum.ERP_PAY.getCode())){
	    				if(StringUtilHelper.isNull(payOrderInfoDMO.getPayStatus()) || !payOrderInfoDMO.getPayStatus().equals(PayTypeEnum.SUCCESS.getCode())){
	    					LOGGER.warn("五合一下行-没有从收付款待下行信息表里查到数据或者收付款支付不是SUCCESS,入参orderNo:"+requestInfo.getOrderNo());
		    				continue;
	    				}
	    				...
	    				LOGGER.warn("五合一下行-没有从收付款待下行信息表里查到数据或者收付款下行状态不是回调成功,入参orderNo:"+requestInfo.getOrderNo());
	    				continue;
	    			}
	    		}
	    		...
	    		for(int i=0;i<orderItemNoArray.length;i++){
	    			...
	    			TradeOrderItemsDMO tradeOrderItemsInfo = tradeOrderErpDistributionDao.selectTradeOrderItemsInfo(tradeOrderItemsDMO);
	    			...
	    			List<TradeOrderItemsWarehouseDetailDMO> tradeOrderItemsWarehouseDetailList = tradeOrderErpDistributionDao.selectTradeOrderItemsWarehouseDetailList(tradeOrderItemsWarehouseDetailDMO);
	    			
	    			BigDecimal rebateAll = new BigDecimal(0);
	    			if(null != tradeOrderItemsWarehouseDetailList && tradeOrderItemsWarehouseDetailList.size()>0){
	    				for(int j=0;j<tradeOrderItemsWarehouseDetailList.size();j++){
			    			...
			    			orderDetailMap.put("warehouseCode", tradeOrderItemsWarehouseDetailList.get(j).getWarehouseCode());
			    			orderDetailMap.put("productCode", tradeOrderItemsInfo.getItemSpuCode());
			    		
			    			orderDetailMap.put("productNum", tradeOrderItemsWarehouseDetailList.get(j).getGoodsCount().toString());//有默认值,无需判空
			    			int isChangePrice = tradeOrderItemsInfo.getIsChangePrice();//是否议价 (1:是，0:否)
			    			if(IS_CHANGE_PRICE_YES == isChangePrice){
			    				orderDetailMap.put("price", df1.format(tradeOrderItemsInfo.getBargainingGoodsPrice()));//有默认值,无需判空
			    			}else{
			    				orderDetailMap.put("price", df1.format(tradeOrderItemsInfo.getGoodsPrice()));//有默认值,无需判空
			    			}
			    			
			    			//使用返利金额（默认0） 求百分比,精确到两位，四舍五入
			    			if(zero != tradeOrderItemsInfo.getGoodsCount()){
			    				BigDecimal warehouseGoodsCount =  new BigDecimal(tradeOrderItemsWarehouseDetailList.get(j).getGoodsCount());
			    				BigDecimal itemsGoodsCount =  new BigDecimal(tradeOrderItemsInfo.getGoodsCount());
			    				LOGGER.info("warehouseGoodsCount:"+warehouseGoodsCount+" itemsGoodsCount:"+itemsGoodsCount+" tradeOrderItemsInfo.getUsedRebateAmount():"+ new BigDecimal(df1.format(tradeOrderItemsInfo.getTotalDiscountAmount())));
			    				
			    				if(j == tradeOrderItemsWarehouseDetailList.size()-1){
			    					BigDecimal rebateTemp =  new BigDecimal(df1.format(tradeOrderItemsInfo.getTotalDiscountAmount())).subtract(rebateAll);
			    					orderDetailMap.put("rebate",rebateTemp);
			    				}else{
			    					BigDecimal rebate = warehouseGoodsCount.divide(itemsGoodsCount, 8, BigDecimal.ROUND_HALF_UP).multiply( new BigDecimal(df1.format(tradeOrderItemsInfo.getTotalDiscountAmount()))).setScale(8, BigDecimal.ROUND_HALF_UP);
			    			...
			    			BigDecimal rebate = new BigDecimal(orderDetailMap.get("rebate").toString());
			    			//销售金额
			    			BigDecimal amount = new BigDecimal(tradeOrderItemsWarehouseDetailList.get(j).getGoodsCount()).multiply(new BigDecimal(orderDetailMap.get("price").toString()));
			    			orderDetailMap.put("payment", df1.format(amount.subtract(rebate)));
			    			
			    			payMent = payMent.add(amount).subtract(rebate);//销售金额（总）
			    			orderDetailMap.put("productAttribute", tradeOrderItemsWarehouseDetailList.get(j).getProductAttribute());//商品属性
			    			orderDetailMap.put("supplierCode", tradeOrderItemsWarehouseDetailList.get(j).getSupplierCode());//供货商
			    			orderDetailMap.put("departmentCode", tradeOrderItemsWarehouseDetailList.get(j).getPurchaseDepartmentCode());//采购部门
			    			orderDetailMap.put("agreementNum", "");//采购协议-数据库里没有
			    			orderDetailList.add(orderDetailMap);
		    			}
	    			}else{
	    				LOGGER.warn("trade_order_items_warehouse_detail表没有查到数据,条件:"+JSONObject.toJSONString(tradeOrderItemsWarehouseDetailDMO));//TODO
	    				continue outterLoop;
	    			}

	    			//tradeOrderItemsInfo.getHasUsedCoupon() 为1 说明使用了优惠券，tradeOrderItemsInfo.getErpRebateNo()不为空说明使用了返利，不能叠加使用
	    			if(tradeOrderItemsInfo.getHasUsedCoupon().intValue() == Integer.valueOf(OrderStatusEnum.HAS_USED_COUPON.getCode()).intValue()
	    				...
	    				rebateAmountTotal = rebateAmountTotal.add(new BigDecimal(df1.format(tradeOrderItemsInfo.getTotalDiscountAmount())));//返利金额（总）
	    				if(StringUtilHelper.isNull(tradeOrderItemsInfo.getErpRebateNo())){//说明用了优惠券
			    			...
			    			rebateDetailMap.put("rebateAmount", df1.format(tradeOrderItemsInfo.getTotalDiscountAmount()));//返利金额(数据库有默认值,不要判空)
			    			rebateDetailMap.put("categoryCode", tradeOrderItemsInfo.getErpFirstCategoryCode());//商品分类代码 返利单号为''时不能为空
			    			rebateDetailMap.put("brandCode", tradeOrderItemsInfo.getBrandId().toString());//品牌代码 返利单号为''时不能为空(数据库有默认值,不要判空)
			    			...
			    			if("".equals(tradeOrderItemsInfo.getErpRebateNo())){//数据库默认''
			    				...
			    				rebateDetailMap.put("rebateCode", tradeOrderItemsInfo.getErpRebateCode());
			    			}
			    			
			    			if(StringUtilHelper.isNotNull(requestInfo.getIsTimelimitedOrder()) &&
			    					OrderStatusEnum.IS_TIMELIMITED_ORDER.getCode().equals(requestInfo.getIsTimelimitedOrder().toString())){
			    				rebateDetailMap.put("rebateIdentifier", requestInfo.getPromotionId());//红包唯一标识符
			    			}else{
			    				String orderNo = requestInfo.getOrderNo();
			    				List<TradeOrderItemsDiscountDMO> itemDiscount = tradeOrderItemsDiscountDAO.selectBuyerCouponCodeByOrderNo(orderNo);
			    				if(null != itemDiscount && itemDiscount.size()>0){
			    					rebateDetailMap.put("rebateIdentifier",itemDiscount.get(0).getPromotionId() );//红包唯一标识符
			    					if(StringUtilHelper.isNull(itemDiscount.get(0).getPromotionId())){
		    			  ...
		    			  rebateDetailList.add(rebateDetailMap);
	    					...
	    					if(VMSRebateMap.containsKey("rebateNo")){
	    						String rebateAmount = df1.format(new BigDecimal(VMSRebateMap.get("rebateAmount").toString())
	    						...
	    						VMSRebateMap.put("rebateAmount", df1.format(tradeOrderItemsInfo.getTotalDiscountAmount()));
	    						VMSRebateMap.put("rebateNo", tradeOrderItemsInfo.getErpRebateNo());
	    					}
	    				}
	    			}
    			...
    			if(orderCompensateERPDTO.getSourceId().equals(OrderStatusEnum.ORDER_FROM_VMS.getCode())){
    			...
    			item_down_erp_template.convertAndSend(mqQueueFactoryConfig.getOrdercenterMiddlewareCompensate(),JSONUtil.toJSONString(orderCompensateERPDTO));
		        LOGGER.info("成功发送mq----五合一队列名:"+mqQueueFactoryConfig.getOrdercenterMiddlewareCompensate());
		        //直接更新订单表
		        changeOrderStatus(requestInfo.getOrderNo(), requestInfo.getOrderItemNos(), OrderStatusEnum.PAYED_SPLITED_ORDER_PRE_ERP.getCode(), 
		        		OrderStatusEnum.PAYED_SPLITED_ORDER_PRE_ERP.getMsg(),false,"","");
	    	}else{
	    		LOGGER.warn("从表trade_order_erp_distribution查出的订单行号(orderItemNos):"+requestInfo.getOrderItemNos()+" "+requestInfo.getOrderNo());
	    	}
			...
			new Thread(new Runnable(){
				public void run(){
					...
					if (ordersItemCount != zero) {
						...
						ordersItemPreDeliveryStatusMap.put("orderItemStatus", OrderStatusEnum.PRE_DELIVERY.getCode());
						long ordersItemPreDeliveryStatusCount = tradeOrderItemsDAO
								.selectTradeOrderItemsByOrderNoOrStatus(ordersItemPreDeliveryStatusMap);
						
						if (ordersItemStatusCount>0 && ordersItemPreDeliveryStatusCount>0 && 
								ordersItemCount == ordersItemStatusCount+ordersItemPreDeliveryStatusCount) {
							orderStatusChangeCommonService.updateOrder(status, orderNo, statusText,orderErrorStatus,orderErrorReason,false);
						}
					}
				}
			}).start();
	...
	public OrderCompensateERPCallBackDMO executeOrderCompensateERPCallBack(OrderCompensateERPCallBackReqDTO orderCompensateERPCallBackReqDTO) {
		...
		LOGGER.info("messageId:{}五合一下行回调开始{}",messageId,JSONObject.toJSONString(orderCompensateERPCallBackReqDTO));
		...
		try{
			TradeOrderErpDistributionDMO tradeOrderErpDistributionDMO = new TradeOrderErpDistributionDMO();
			tradeOrderErpDistributionDMO.setErpLockBalanceCode(orderCompensateERPCallBackReqDTO.getOrderCode());
			tradeOrderErpDistributionDMO.setErpFirstCategoryCode(orderCompensateERPCallBackReqDTO.getCategroyCode());
			tradeOrderErpDistributionDMO.setBrandId(Long.parseLong(orderCompensateERPCallBackReqDTO.getBrandCode()));
			
			TradeOrderErpDistributionDMO selectTradeOrderErpDistributionDMO = tradeOrderErpDistributionDAO.selectOrderItemNosBySelective(tradeOrderErpDistributionDMO);
			LOGGER.info("messageId:{}五合一下行回调--查询分销单表结果:{}",messageId,JSONObject.toJSONString(selectTradeOrderErpDistributionDMO));
			if(null == selectTradeOrderErpDistributionDMO || "".equals(selectTradeOrderErpDistributionDMO.getOrderNo())){
				LOGGER.info("中间件回调订单中心时候,没有查到订单分销单信息表的数据-入参:"+JSONObject.toJSONString(tradeOrderErpDistributionDMO));
				orderCompensateERPCallBackDMO.setResultCode(ResultCodeEnum.ORDERCALLBACK_QUERY_ERP_DISTRIBUTION_IS_NULL.getCode());
				orderCompensateERPCallBackDMO.setResultMsg(ResultCodeEnum.ORDERCALLBACK_QUERY_ERP_DISTRIBUTION_IS_NULL.getMsg());
			...
			if(orderCompensateERPCallBackReqDTO.getResult().equals(MiddleWareEnum.SUCCESS_ONE.getCode())){//1：成功
				tradeOrderErpDistributionDMO.setErpSholesalerCode(orderCompensateERPCallBackReqDTO.getJL_SholesalerCode());
				tradeOrderErpDistributionDMO.setErpWholesalePaymentCode(orderCompensateERPCallBackReqDTO.getJL_WholesalePayment());
				tradeOrderErpDistributionDMO.setErpCreateRecordCode(orderCompensateERPCallBackReqDTO.getJL_CreateRecordCode());
				tradeOrderErpDistributionDMO.setErpCompanyCode(orderCompensateERPCallBackReqDTO.getJL_ComPanyCode());
				...
				tradeOrderErpDistributionDAO.updateTradeOrderErpDistribution(tradeOrderErpDistributionDMO);
				
				tradeOrderItemsDMO.setErpSholesalerCode(orderCompensateERPCallBackReqDTO.getJL_SholesalerCode());
				...
				LOGGER.info("messageId :{}五合一回调成功,从分销单表里查询出的订单行集合为：{},订单号{}",messageId,orderItemNos,orderNo);
				TradeOrdersDMO tradeOrdersDMOTemp = tradeOrdersDAO.selectOrderByOrderNoANDOrderItemNo(orderQueryParamDMO);
				LOGGER.info("messageId :{}五合一回调成功，判断是不是要扣减库存查询结果tradeOrdersDMOTemp{}",messageId,JSONObject.toJSONString(tradeOrdersDMOTemp));
				if(null != tradeOrdersDMOTemp && null != tradeOrdersDMOTemp.getIsTimelimitedOrder() && 
					...
					LOGGER.info("messageId :{}五合一回调成功，扣减非秒杀商品 isTimelimitedOrder{}",messageId,tradeOrdersDMOTemp.getIsTimelimitedOrder());
				...
				tradeOrderErpDistributionLogDMO.setErpDistributionId(selectTradeOrderErpDistributionDMO.getId());
				tradeOrderErpDistributionLogDMO.setErpErrorMsg(orderCompensateERPCallBackReqDTO.getErrormessage());
				...
				long updateErpDistributionLog = tradeOrderErpDistributionLogDAO.updateErpDistributionLogByPrimaryKey(tradeOrderErpDistributionLogDMO);
				if(updateErpDistributionLog==0){//插入
					tradeOrderErpDistributionLogDMO.setCreateId(Long.parseLong(Constant.OPERATE_CODE));
					...
					tradeOrderErpDistributionLogDAO.insertErpDistributionLog(tradeOrderErpDistributionLogDMO);
				}
				
				handleOrderAndItemParam(tradeOrderItemsDMO,tradeOrdersDMO,orderCompensateERPCallBackReqDTO);
			}
			//更新订单表和订单行表
			handleOrderAndItem(selectTradeOrderErpDistributionDMO,tradeOrderItemsDMO,selectTradeOrderErpDistributionDMO.getOrderNo());
	 ...
	 * 五合一回调-更新订单表和订单行
	 */
	private void handleOrderAndItem(TradeOrderErpDistributionDMO selectTradeOrderErpDistributionDMO,TradeOrderItemsDMO tradeOrderItemsDMO,String orderNo){
		...
		if(StringUtilHelper.isNotNull(orderItemNos)){
			...
			for(int i=0;i<orderItemNosArray.length;i++){
				...
				if(OrderStatusEnum.PRE_DELIVERY.getCode().equals(tradeOrderItemsDMO.getOrderItemStatus())){
					//插入订单行履历表
			...
			if(ordersItemCount != zero){
				...
				if(ordersItemCount == ordersItemStatusCount){
					...
					if(null != querTradeOrders && StringUtilHelper.isNotNull(querTradeOrders.getOrderFrom()) 
							&& OrderStatusEnum.ORDER_FROM_VMS.getCode().equals(querTradeOrders.getOrderFrom())){
	 ...
	 */
	private void handleOrderAndItemParam(TradeOrderItemsDMO tradeOrderItemsDMO,TradeOrdersDMO tradeOrdersDMO,OrderCompensateERPCallBackReqDTO orderCompensateERPCallBackReqDTO){
		//如果是VMS开单下行失败订单总订单状态改为“待支付”，子订单状态改为“待支付-VMS开单待下行ERP”
		TradeOrdersDMO tradeOrdersRes = tradeOrdersDAO.selectOrderByOrderNo(tradeOrdersDMO.getOrderNo());
		if(StringUtilHelper.isNotNull(tradeOrdersRes.getOrderFrom()) && tradeOrdersRes.getOrderFrom().equals(OrderStatusEnum.ORDER_FROM_VMS.getCode())){
		...
		tradeOrderItemsDMO.setOrderItemErrorReason(orderCompensateERPCallBackReqDTO.getErrormessage());
	...
	public int updateTradeOrderErpDistributionByPrimaryKey(
		...
		int update = tradeOrderErpDistributionDAO.updateTradeOrderErpDistributionByPrimaryKey(tradeOrderErpDistributionDMO);
	...
	private EmptyResDTO batchReduceStock(final TradeOrdersDMO record, final String messageId) {
		...
		try {
			...
			OtherCenterResDTO<String> result = goodsCenterRAO
					.comboChangeStock(order4StockChangeDTOs, messageId);
