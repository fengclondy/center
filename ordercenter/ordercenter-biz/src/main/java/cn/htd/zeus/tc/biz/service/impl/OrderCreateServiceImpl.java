...
public class OrderCreateServiceImpl implements OrderCreateService {
	...
	public OrderCreateInfoDMO orderCreate(OrderCreateInfoReqDTO orderCreateInfoReqDTO) {
		...
		if(StringUtilHelper.isNotNull(couponCode) && StringUtilHelper.isNull(orderCreateInfoReqDTO.getPromotionType())){
		...
		isNeedBatchReserveStockMap.put(IS_NEED_BATCH_RESERVE_STOCK_KEY, OrderStatusEnum.ORDER_NOT_RELEASE_STOCK.getCode());
		...
		try {
			...
			if (null != orderList && orderList.size() > 0) {
				...
				if (StringUtilHelper.isNotNull(promotionType)) {
					...
					orderCreateInfoReqDTO = usePromotion(orderCreateInfoReqDTO,orderCreateInfoDMO,order4StockChangeDTOs,isNeedBatchReserveStockMap,orderItemPromotionList,buyerInfoDTO);
					if(!ResultCodeEnum.SUCCESS.getCode().equals(orderCreateInfoDMO.getResultCode())){
						return orderCreateInfoDMO;
					}
				} 
				orderCreateInfoDMO = preInsertOrderAndOrderItem(buyerInfoDTO,
						orderCreateInfoReqDTO, orderCreateInfoDMO);
			}
		} catch (Exception e) {
			...
			if(orderCreateInfoDMO.getResultCode().equals(ResultCodeEnum.ORDER_CREATE_ORDER_ROLLBACK_BATCHRELEASE.getCode())){
				if(StringUtilHelper.isNotNull(promotionType)){
					//释放商品和优惠券库存
					batchReleaseGoodsAndMarketStock(order4StockChangeDTOs,orderItemPromotionList,promotionType,isNeedBatchReserveStockMap,orderCreateInfoReqDTO.getMessageId());
				}else{
					//释放商品库存
					batchReleaseGoodsAndMarketStock(order4StockChangeDTOs,null,null,isNeedBatchReserveStockMap,orderCreateInfoReqDTO.getMessageId());
				}
			}
		}
	...
	/*
	 * 使用优惠券或者秒杀--组装参数
	 * java 是指传递 所以返回orderCreateInfoReqDTO
	 * orderCreateInfoReqDTO = JSONObject.toJavaObject(jsonObjRes,OrderCreateInfoReqDTO.class); 
	 */
	private OrderCreateInfoReqDTO usePromotion(OrderCreateInfoReqDTO orderCreateInfoReqDTO,OrderCreateInfoDMO orderCreateInfoDMO,
			List<Order4StockChangeDTO> order4StockChangeDTOs,Map<String,String> isNeedBatchReserveStockMap,
		...
		OtherCenterResDTO<TradeInfoDTO> calculateRes = marketCenterRAO.calculateCouponDiscount(couponCodeList, cart, orderCreateInfoReqDTO.getMessageId());
		...
		OtherCenterResDTO<String> reserveBuyerPromotionRes = marketCenterRAO.reserveBuyerPromotion(orderItemPromotionList,orderCreateInfoReqDTO.getMessageId());
		if(!ResultCodeEnum.SUCCESS.getCode()
	 ...
	 * 
	 */
	private OrderCreateInfoDMO handleReqDTO(OrderCreateInfoReqDTO orderCreateInfoReqDTO, OrderCreateInfoDMO orderCreateInfoDMO,List<Order4StockChangeDTO> order4StockChangeDTOs,Map<String,String> isNeedBatchReserveStockMap) {
		orderCreateInfoDMO.setResultCode(ResultCodeEnum.SUCCESS.getCode());
		List<OrderCreateListInfoReqDTO> orderList = orderCreateInfoReqDTO.getOrderList();
		
		//秒杀商品不需要锁定商品中心商品,也不需要校验商品库存
		String promotionType = orderCreateInfoReqDTO.getPromotionType()==null?"":orderCreateInfoReqDTO.getPromotionType();
		...
		for (int i = 0; i < orderList.size(); i++) {
			...
			if (null != orderItemList && orderItemList.size() > 0) {
				for (int j = 0; j < orderItemList.size(); j++) {
					...
					OtherCenterResDTO<MallSkuWithStockOutDTO> mallSkuWithStockOutResDTO = goodsCenterRAO
							.queryMallItemDetailWithStock(orderItemTemp,site,orderCreateInfoReqDTO.getMessageId());
					if (!ResultCodeEnum.SUCCESS.getCode()
						...
						orderCreateInfoDMO
								.setResultMsg(mallSkuWithStockOutResDTO.getOtherCenterResponseMsg());
						return orderCreateInfoDMO;
					}
					...
					if(promotionType.equals(OrderStatusEnum.PROMOTION_TYPE_SECKILL.getCode())){
						...
						if(null != mallSkuStockOutDTO.getIsUpShelf() && mallSkuStockOutDTO.getIsUpShelf() == 0){
							orderCreateInfoDMO.setResultCode(ResultCodeEnum.ORDERSETTLEMENT_PRODUCT_IS_DOWN_SHELF.getCode());
							orderCreateInfoDMO.setResultMsg(ResultCodeEnum.ORDERSETTLEMENT_PRODUCT_IS_DOWN_SHELF.getMsg());
							return orderCreateInfoDMO;
						}
						
						// 去价格中心查询商品价格
						OtherCenterResDTO<OrderItemSkuPriceDTO> commonItemSkuPriceResDTO = queryItemSkuPrice(mallSkuOutDTO,
								orderCreateInfoReqDTO, orderTemp, orderItemTemp,buyerGradeMap,
								orderCreateInfoReqDTO.getMessageId());
						if (!ResultCodeEnum.SUCCESS.getCode()
								.equals(commonItemSkuPriceResDTO.getOtherCenterResponseCode())) {
							orderCreateInfoDMO.setResultCode(commonItemSkuPriceResDTO.getOtherCenterResponseCode());
							orderCreateInfoDMO.setResultMsg(commonItemSkuPriceResDTO.getOtherCenterResponseMsg());
							return orderCreateInfoDMO;
						}
					...
					orderItemTemp.setGoodsPrice(orderItemSkuPriceDTO.getGoodsPrice()==null?new BigDecimal(0):orderItemSkuPriceDTO.getGoodsPrice());
					...
					if (StringUtilHelper.isNotNull(mallSkuOutDTO.getProductChannelCode())
						...
						orderTemp.setHasProductplusFlag(Integer.valueOf(OrderStatusEnum.HAS_PRODUCTPLUS_FLAG.getCode()));
						orderItemTemp.setGoodsFreight(mallSkuOutDTO.getFreightAmount()==null?new BigDecimal(0):mallSkuOutDTO.getFreightAmount());
						...
						orderItemTemp.setSalePrice(orderItemSkuPriceDTO.getSalePrice()==null?new BigDecimal(0):orderItemSkuPriceDTO.getSalePrice());
						
						JDAcountAmt = JDAcountAmt.add(orderItemTemp.getGoodsPrice().multiply(new BigDecimal(orderItemTemp.getGoodsCount())));
						
						//汇通达在京东备用金
						accountAmount4JDRes = goodsPlusRAO.queryAccountAmount4JD(orderCreateInfoReqDTO.getMessageId());
						if(!accountAmount4JDRes.getOtherCenterResponseCode().equals(ResultCodeEnum.SUCCESS.getCode())){
							orderCreateInfoDMO.setResultCode(accountAmount4JDRes.getOtherCenterResponseCode());
							orderCreateInfoDMO.setResultMsg(accountAmount4JDRes.getOtherCenterResponseMsg());
							return orderCreateInfoDMO;
						}else{
							BigDecimal bd = new BigDecimal(accountAmount4JDRes.getOtherCenterResult());
					    ...
					    skuList.add(sku);
							...
							&& mallSkuOutDTO.getProductChannelCode().equals(GoodCenterEnum.EXTERNAL_SUPPLIER.getCode()))){
						//校验外部供应商的商品数量是否大于提交数量--秒杀不需要校验
						if(!promotionType.equals(OrderStatusEnum.PROMOTION_TYPE_SECKILL.getCode())){
						    orderCreateInfoDMO = validateInternalSupplierQuantity(mallSkuStockOutDTO,orderItemTemp,orderCreateInfoDMO);
						    isNeedBatchReserveStockMap.put(IS_NEED_BATCH_RESERVE_STOCK_KEY, OrderStatusEnum.ORDER_RELEASE_STOCK.getCode());
						}
						if(!orderCreateInfoDMO.getResultCode().equals(ResultCodeEnum.SUCCESS.getCode())){
							return orderCreateInfoDMO;
						}
						
						orderItemTemp.setGoodsFreight(new BigDecimal(0));//TODO 2017-02-08 外部供应商运费先写成0,等蒋坤提供运费接口(外部供应商是用运费模板，然后计算)
					
						// 如果是外部供应商-将saleprice不用赋值，计算阶梯价格计算goodprice即可
						BigDecimal goodsPrice = new ExternalSupplierCostCaculateUtil().caculateLadderPrice4outer(orderItemTemp.getGoodsCount().intValue(),
					...
					//组装填充订单行数据
					assembleOrderItem(orderItemTemp,orderItemSkuPriceDTO,mallSkuOutDTO,buyerGradeMap);
					if (StringUtilHelper
						...
						orderItemTemp.setIsOutDistribtion(
							...
							&& (mallSkuOutDTO.getProductChannelCode().equals(GoodCenterEnum.EXTERNAL_SUPPLIER.getCode()))
							|| mallSkuOutDTO.getProductChannelCode().equals(GoodCenterEnum.JD_SUPPLIER.getCode()) )){
						orderItemTemp.setIsOutDistribtion(
								Integer.valueOf(OrderStatusEnum.NOT_OUT_DISTRIBTION.getCode()));
					}else {
						OtherCenterResDTO<String> isRecevieAddressInSaleRangeRes = goodsCenterRAO
								.isRecevieAddressInSaleRange(orderCreateInfoReqDTO, orderItemTemp,
										orderCreateInfoReqDTO.getMessageId());
						if(!ResultCodeEnum.SUCCESS.getCode().equals(isRecevieAddressInSaleRangeRes.getOtherCenterResponseCode())){
							orderCreateInfoDMO.setResultCode(isRecevieAddressInSaleRangeRes.getOtherCenterResponseCode());
							orderCreateInfoDMO.setResultMsg(isRecevieAddressInSaleRangeRes.getOtherCenterResponseMsg());
							return orderCreateInfoDMO;
						}
					}
					...
					if(StringUtilHelper.isNotNull(mallSkuOutDTO.getProductChannelCode())
						...
						order4StockEntryDTO
						.setQuantity(Integer.valueOf(orderItemTemp.getGoodsCount().toString()));
			...
			if (null != orderTemp.getHasProductplusFlag()) {
				...
				OtherCenterResDTO<String> selectChannelAddressDTO = memberCenterRAO.selectChannelAddressDTO(orderCreateInfoReqDTO.getMessageId(),
						orderTemp.getSellerCode(),Constant.PRODUCT_CHANNEL_CODE_OUTLINE);
				if(!selectChannelAddressDTO.getOtherCenterResponseCode().equals(ResultCodeEnum.SUCCESS.getCode())){
					orderCreateInfoDMO.setResultCode(selectChannelAddressDTO.getOtherCenterResponseCode());
					orderCreateInfoDMO.setResultMsg(selectChannelAddressDTO.getOtherCenterResponseMsg());
					return orderCreateInfoDMO;
				}
				batchGetStockReqDTO.setArea(selectChannelAddressDTO.getOtherCenterResult());
				orderCreateInfoDMO = validateJDGoodsStock(batchGetStockReqDTO,orderCreateInfoDMO,orderCreateInfoReqDTO.getMessageId());
				if(!orderCreateInfoDMO.getResultCode().equals(ResultCodeEnum.SUCCESS.getCode())){
					return orderCreateInfoDMO;
				}
				
				BigDecimal accountAmount4JD = new BigDecimal(accountAmount4JDRes.getOtherCenterResult());
				if(accountAmount4JD.compareTo(JDAcountAmt)<ZERO){
					orderCreateInfoDMO.setResultCode(ResultCodeEnum.JD_RESERVE_FUND_NOT_ENOUGH.getCode());
					orderCreateInfoDMO.setResultMsg(ResultCodeEnum.JD_RESERVE_FUND_NOT_ENOUGH.getMsg());
					return orderCreateInfoDMO;
				}
				//创建京东订单 --3
				OrderCreateInfoDMO jdCreateRes = jdCreateOrderService.createJDOrder(orderCreateInfoReqDTO.getMessageId(),jdCreateOrderReqDTO,orderTemp);
			...
			if(externalSupplierFlag){
				...
				orderFreightInfoService.calculateOrderItemFeight4CreateOrder(orderItemList, orderCreateInfoReqDTO.getSite());
		...
		// 调用商品中心批量锁定库存
		if(OrderStatusEnum.ORDER_RELEASE_STOCK.getCode().equals(isNeedBatchReserveStockMap.get(IS_NEED_BATCH_RESERVE_STOCK_KEY))){
	 ...
	 * 组装订单行数据
	 */
	private void assembleOrderItem(OrderCreateItemListInfoReqDTO orderItemTemp,OrderItemSkuPriceDTO orderItemSkuPriceDTO,MallSkuOutDTO mallSkuOutDTO,Map<String,String> buyerGradeMap){
		orderItemTemp.setCostPrice(orderItemSkuPriceDTO.getCostPrice()==null?new BigDecimal(0):orderItemSkuPriceDTO.getCostPrice());
		orderItemTemp.setPriceFloatingRatio(orderItemSkuPriceDTO.getPriceFloatingRatio()==null?new BigDecimal(0):orderItemSkuPriceDTO.getPriceFloatingRatio());
		orderItemTemp.setCommissionRatio(orderItemSkuPriceDTO.getCommissionRatio()==null?new BigDecimal(0):orderItemSkuPriceDTO.getCommissionRatio());
		orderItemTemp.setGoodsPriceType(orderItemSkuPriceDTO.getGoodsPriceType()==null?"":orderItemSkuPriceDTO.getGoodsPriceType().toString());
		String buyerGrade = buyerGradeMap.get("buyerGrade")==null?"":buyerGradeMap.get("buyerGrade").toString();
	...
	private OrderCreateInfoDMO preInsertOrderAndOrderItem(MemberBaseInfoDTO buyerInfoDTO,
		...
		try{
			...
			int isOutDistribtion = Integer.parseInt(OrderStatusEnum.NOT_OUT_DISTRIBTION.getCode());// 是否超出配送范围// 0:否,1:-是(返回给前台页面展示)
			
			for (int i = 0; i < orderList.size(); i++) {
				OrderCreateListInfoDMO orderCreateListInfoDMO = new OrderCreateListInfoDMO();// 创建订单返回结果(提供前台待支付
																								// )-2
				int isOrderOutDistribtion = Integer.parseInt(OrderStatusEnum.NOT_OUT_DISTRIBTION.getCode());// 订单是否超出配送范围// 0:否,1:-是
				...
				if (null != orderItemList && orderItemList.size() > 0) {
					for (int j = 0; j < orderItemList.size(); j++) {
						...
						externalAmt = assemblePrePayAndPreSavePayOrderInfo(brandIdErpFirstCategoryCode,tradeOrderItemsDMOMap,orderItemTemp,
								tradeOrderItemsDMO,orderTemp,orderCreateInfoReqDTO.getBuyerCode(),externalAmt,chargeConditionInfoDMOList
								,orderCreateInfoReqDTO.getMessageId());
						
						if(OrderStatusEnum.OUT_DISTRIBTION.getCode().equals(tradeOrderItemsDMO.getIsOutDistribtion()==null?"":tradeOrderItemsDMO.getIsOutDistribtion().toString())){
						...
						totalGoodsAmount = totalGoodsAmount.add(tradeOrderItemsDMO.getGoodsAmount());
						totalDiscountAmount = totalDiscountAmount
								.add(orderItemTemp.getTotalDiscountAmount() == null ? new BigDecimal(0)
						...
						orderPayAmount = orderPayAmount.add(tradeOrderItemsDMO.getOrderItemPayAmount());
						// TODO 运费总金额 total_freight
					}
				}
				// 插入订单表
				Map<String, Object> orderParam = new HashMap<String, Object>();
				assembleOrderParam(orderParam,orderTemp.getHasProductplusFlag() , totalGoodsCount, totalGoodsAmount,
						platformDiscountAmount, orderTotalAmount, orderPayAmount, isOrderOutDistribtion);
				OrderCreateInfoDMO insetTradeOrdersRes = insetTradeOrders(buyerInfoDTO,
						orderCreateInfoReqDTO, orderTemp, orderParam, orderCreateInfoDMO,totalFreight);
				
				// 按照品牌品类拼装给前台3
				assemblePrePayAndSavePayOrderInfo(tradeOrderItemsDMOMap,orderTemp,chargeConditionInfoDMOList,orderCreateInfoReqDTO.getBuyerCode(),
						externalAmt,orderCreateListInfoDMO,orderCreateInfoReqDTO.getPromotionType(),orderCreateInfoReqDTO.getMessageId());
			}
			...
			orderCreateInfoDMO.setResultCode(ResultCodeEnum.ORDER_CREATE_ORDER_ROLLBACK_BATCHRELEASE.getCode());
			orderCreateInfoDMO.setResultMsg(ResultCodeEnum.ORDER_CREATE_ORDER_ROLLBACK_BATCHRELEASE.getMsg());
			StringWriter w = new StringWriter();
			e.printStackTrace(new PrintWriter(w));
	...
	private void batchReleaseGoodsAndMarketStock(List<Order4StockChangeDTO> order4StockChangeDTOs,
			List<OrderItemPromotionDTO> orderItemPromotionList,String promotionType,Map<String,String> isNeedBatchReserveStockMap,String messageId){
		try{
			if(OrderStatusEnum.ORDER_RELEASE_STOCK.getCode().equals(isNeedBatchReserveStockMap.get(IS_NEED_BATCH_RESERVE_STOCK_KEY))){
				//释放商品库存
			...
			StringWriter w = new StringWriter();
			e.printStackTrace(new PrintWriter(w));
			LOGGER.error("MessageId:{} 调用方法(释放商品和优惠券、秒杀库存)OrderCreateServiceImpl.batchReleaseGoodsAndMarketStock出现异常{}",
	 ...
	 */
	private BigDecimal assemblePrePayAndPreSavePayOrderInfo(String brandIdErpFirstCategoryCode,Map<String,BigDecimal> tradeOrderItemsDMOMap,
			OrderCreateItemListInfoReqDTO orderItemTemp,TradeOrderItemsDMO tradeOrderItemsDMO,OrderCreateListInfoReqDTO orderTemp,
			String buyerCode,BigDecimal externalAmt,List<ChargeConditionInfoDMO> chargeConditionInfoDMOList,String messageId) throws Exception{
		
		//外部供应商不需要按照品牌品类返回给支付,但是具有内部供应商身份的外部供应商要插入到收付款表
		if(StringUtilHelper.isNotNull(orderItemTemp.getChannelCode())
				&& orderItemTemp.getChannelCode().equals(GoodCenterEnum.EXTERNAL_SUPPLIER.getCode())){
			    OtherCenterResDTO<String> memberRes = memberCenterRAO.isHasInnerComapanyCert(orderTemp.getSellerCode(), messageId);
				if(ResultCodeEnum.SUCCESS.getCode().equals(memberRes.getOtherCenterResponseCode()) && 
						memberRes.getOtherCenterResult().equals(MemberCenterEnum.HAS_INNER_COMAPANY_CERT.getCode())){
					preSavePayOrderInfo(orderItemTemp,brandIdErpFirstCategoryCode,tradeOrderItemsDMOMap,tradeOrderItemsDMO);
				}
				ChargeConditionInfoDMO chargeConditionInfoDMO = new ChargeConditionInfoDMO();
				chargeConditionInfoDMO.setBrandCode(orderItemTemp.getBrandId()==null?"":orderItemTemp.getBrandId().toString());
	...
	public void preSavePayOrderInfo(OrderCreateItemListInfoReqDTO orderItemTemp,String brandIdErpFirstCategoryCode,
			Map<String,BigDecimal> tradeOrderItemsDMOMap,TradeOrderItemsDMO tradeOrderItemsDMO)throws Exception{
		if (StringUtilHelper.isNotNull(orderItemTemp.getChannelCode())
				&& orderItemTemp.getChannelCode()
						.startsWith(GoodCenterEnum.EXTERNAL_CHANNELS.getCode())) {
			 brandIdErpFirstCategoryCode = jdConfig.getJdBrandCode() + "#"
						+ jdConfig.getJdClassCode();
		}else if(StringUtilHelper.isNotNull(orderItemTemp.getChannelCode())
				&& orderItemTemp.getChannelCode().equals(GoodCenterEnum.EXTERNAL_SUPPLIER.getCode())){
			brandIdErpFirstCategoryCode = orderItemTemp.getBrandId()==null?"":orderItemTemp.getBrandId().toString() + "#"
					+ GoodCenterEnum.EXTERNAL_SUPPLIER.getCode();
		}else if(StringUtilHelper.isNotNull(orderItemTemp.getChannelCode())
				&& orderItemTemp.getChannelCode().equals(GoodCenterEnum.INTERNAL_SUPPLIER.getCode())){
			 ...
			 if(StringUtilHelper.isNull(orderItemTemp.getErpFirstCategoryCode())){
				 LOGGER.error("内部供应商商品没有查到erp一级类目ErpFirstCategoryCode:"+orderItemTemp.getErpFirstCategoryCode());
				 throw new Exception();
			 }
		}
		...
		if (tradeOrderItemsDMOMap.containsKey(brandIdErpFirstCategoryCode)) {
			BigDecimal chargeAmount = tradeOrderItemsDMOMap
					.get(brandIdErpFirstCategoryCode)
	 ...
	 * 1 组装参数返回给前台支付用
	 * 2 组装参数插入收付款待下行信息表 
	 * 3外部供应商商品不需要插入收付款待下行信息表 
	 */
	private void assemblePrePayAndSavePayOrderInfo(Map<String, BigDecimal> tradeOrderItemsDMOMap,OrderCreateListInfoReqDTO orderTemp,
			List<ChargeConditionInfoDMO> chargeConditionInfoDMOList,String buyerCode,BigDecimal externalAmt,OrderCreateListInfoDMO orderCreateListInfoDMO,
			...
			for (Map.Entry<String, BigDecimal> m : tradeOrderItemsDMOMap.entrySet()) {
				...
				if(null != itemList && itemList.size()>0){
					...
					/*if(brandId.equals(MiddleWareEnum.JD_BRAND_ID_PAY.getCode())
							...
							|| GoodCenterEnum.IS_VIP_GOODS.getCode().equals(item.getIsVipItem().toString())){
						chargeConditionInfoDMO.setItemOrderNo(orderTemp.getOrderNo());
						LOGGER.info("京东商品锁定用订单号"+chargeConditionInfoDMO.getItemOrderNo());
					}else if(GoodCenterEnum.IS_VIP_GOODS.getCode().equals(item.getIsVipItem().toString())){
						chargeConditionInfoDMO.setItemOrderNo(orderTemp.getOrderNo());
					...
					if(brandId.equals(MiddleWareEnum.JD_BRAND_ID_PAY.getCode())){
						...
						record.setDownOrderNo(orderTemp.getOrderNo());//京东商品锁定余额编号 就用订单号,预售也用订单号下行
						channelCode = GoodCenterEnum.JD_SUPPLIER.getCode();
						chargeConditionInfoDMO.setItemOrderNo(orderTemp.getOrderNo());
						LOGGER.info("京东商品锁定用订单号"+chargeConditionInfoDMO.getItemOrderNo());
					}else if(item.getChannelCode().equals(GoodCenterEnum.EXTERNAL_SUPPLIER.getCode())){
						record.setBrandCode(MiddleWareEnum.JD_BRAND_ID_ERP.getCode());
						record.setClassCode(MiddleWareEnum.JD_CLASS_CODE_ERP.getCode());
						record.setDownOrderNo(lockAmoutNo);//具有内部供应商身份的外部供应商
						channelCode = GoodCenterEnum.EXTERNAL_SUPPLIER.getCode();
						record.setProductCode(MiddleWareEnum.PRODUCTCODE_ERP_EXTERNAL_SUPPLIER.getCode());//平台公司在POP交易
						orderCreateListInfoDMO.setTradeScene(PayStatusEnum.NOT_SUPPORT_ERP_BLANCE_PAY.getCode());
						OtherCenterResDTO<MemberBaseInfoDTO> memberRes = memberCenterRAO.getInnerInfoByOuterHTDCode(orderTemp.getSellerCode(), messageId);
						if(ResultCodeEnum.SUCCESS.getCode().equals(memberRes.getOtherCenterResponseCode())){
							record.setSupplierCode(memberRes.getOtherCenterResult().getMemberCode());
						}
					}else if(GoodCenterEnum.IS_VIP_GOODS.getCode().equals(item.getIsVipItem().toString())){
					...
					int update = payOrderInfoDAO.updatePayorderinfoByOrderNoBrandIdClassCode(record);
					if(update ==  ZERO){								
		...
		orderCreateListInfoDMO.setAmount(amount.compareTo(new BigDecimal(0))==0?externalAmt:amount);
		...
		if(StringUtilHelper.isNotNull(promotionType) && promotionType.equals(OrderStatusEnum.PROMOTION_TYPE_SECKILL.getCode())){
	 ...
	 * 组装参数给外部供应商计算运费用
	 */
	private void assembleFreight(OrderCreateItemListInfoReqDTO orderItemTemp,MallSkuOutDTO mallSkuOutDTO){
	...
	private OtherCenterResDTO<OrderItemSkuPriceDTO> queryItemSkuPrice(MallSkuOutDTO mallSkuOutDTO,
			...
			OrderCreateItemListInfoReqDTO orderItemTemp, Map<String,String> buyerGradeMap, String messageId) {
		...
		queryCommonItemSkuPriceDTO.setBuyerGrade(memberGroupDTO.getOtherCenterResult().getBuyerGrade());
		buyerGradeMap.put("buyerGrade", queryCommonItemSkuPriceDTO.getBuyerGrade());
		queryCommonItemSkuPriceDTO.setMemberGroupId(memberGroupDTO.getOtherCenterResult().getGroupId());
		...
		if(Constant.PRODUCT_CHANNEL_CODE_OUTLINE.equals(queryCommonItemSkuPriceDTO.getItemChannelCode())) {
		    OtherCenterResDTO<Long> memberInfo = memberCenterRAO.getMemberIdByCode(orderTemp.getSellerCode(), messageId);
		    if(!ResultCodeEnum.SUCCESS.getCode().equals(memberInfo.getOtherCenterResponseCode())){
		    	orderItemSkuPriceDTORes.setOtherCenterResponseCode(memberInfo.getOtherCenterResponseCode());
				orderItemSkuPriceDTORes.setOtherCenterResponseMsg(memberInfo.getOtherCenterResponseMsg());
				return orderItemSkuPriceDTORes;
			}
		    Long memberId = memberInfo.getOtherCenterResult();
			OtherCenterResDTO<Boolean> flag = goodsCenterRAO.canProductPlusSaleBySeller(
					memberId, queryCommonItemSkuPriceDTO.getItemChannelCode(),
					mallSkuOutDTO.getThirdCategoryId(),
					mallSkuOutDTO.getBrandId(), messageId);
			queryCommonItemSkuPriceDTO.setIsCanSellProdplusItem(flag.getOtherCenterResult() ? 1 : 0);
		}
	 ...
	 * 准备锁定优惠券和秒杀
	 */
	private void preLockCoupon(OrderCreateInfoReqDTO orderCreateInfoReqDTO,List<OrderItemPromotionDTO> orderItemPromotionList,MemberBaseInfoDTO buyerInfoDTO){
		List<OrderCreateListInfoReqDTO> orderList = orderCreateInfoReqDTO.getOrderList();
		for (int i = 0; i < orderList.size(); i++) {
			OrderCreateListInfoReqDTO orderTemp = orderList.get(i);
			List<OrderCreateItemListInfoReqDTO> orderItemList = orderTemp.getOrderItemList();
	 ...
	 */
	private  OrderCreateInfoDMO validateInternalSupplierQuantity(MallSkuStockOutDTO mallSkuStockOutDTO,OrderCreateItemListInfoReqDTO orderItemTemp,OrderCreateInfoDMO orderCreateInfoDMO){
		orderCreateInfoDMO.setResultCode(ResultCodeEnum.SUCCESS.getCode());
		
		if(null == mallSkuStockOutDTO.getItemSkuPublishInfo()){
			//没有库存信息，或者区域和包厢的已经下架
			orderCreateInfoDMO.setResultCode(ResultCodeEnum.GOODSCENTER_NOT_STOCKINFO_OR_GOODS_OFF_SHELF.getCode());
			orderCreateInfoDMO.setResultMsg(ResultCodeEnum.GOODSCENTER_NOT_STOCKINFO_OR_GOODS_OFF_SHELF.getMsg());
			return orderCreateInfoDMO;
		}
		Integer displayQuantity = mallSkuStockOutDTO.getItemSkuPublishInfo()
				.getDisplayQuantity();// 显示库存，可用库存
		Integer reserveQuantity = mallSkuStockOutDTO.getItemSkuPublishInfo()
				.getReserveQuantity(); // 锁定库存
		...
		if (displayQuantity - reserveQuantity - preReserveQuantity < ZERO) {
			orderCreateInfoDMO.setResultCode(
					ResultCodeEnum.GOODSCENTER_QUERY_SKUINFO_STOCK_LESS_THAN_ZERO
							.getCode());
			orderCreateInfoDMO.setResultMsg(
					ResultCodeEnum.GOODSCENTER_QUERY_SKUINFO_STOCK_LESS_THAN_ZERO
							.getMsg());
	 ...
	 *检验京东库存是否足够 
	 */
	private OrderCreateInfoDMO validateJDGoodsStock(BatchGetStockReqDTO batchGetStockReqDTO,OrderCreateInfoDMO orderCreateInfoDMO,String messageId){
		orderCreateInfoDMO.setResultCode(ResultCodeEnum.SUCCESS.getCode());
		OtherCenterResDTO<String> otherCenterRes = goodsPlusRAO.queryStock4JD(batchGetStockReqDTO, messageId);
	...
	private OrderCreateInfoDMO insetTradeOrders(MemberBaseInfoDTO buyerInfoDTO,
			OrderCreateInfoReqDTO orderCreateInfoReqDTO, OrderCreateListInfoReqDTO orderTemp,
			Map<String, Object> orderParam, OrderCreateInfoDMO orderCreateInfoDMO,BigDecimal totalFreight) {
		...
		// 调用 memberCallCenterService查询会员信息,不用判读是否成功,如果会员中心没有查到数据,直接让程序抛出异常,可以回滚插入的数据
		String messageId = orderCreateInfoReqDTO.getMessageId();
		OtherCenterResDTO<MemberBaseInfoDTO> memberBaseInfoResDTO = memberCenterRAO
				.selectMemberBaseName(orderTemp.getSellerCode(),
						MemberCenterEnum.MEMBER_TYPE_SELLER.getCode(),
						messageId);
		...
		tradeOrdersDMO.setTotalDiscountAmount(new BigDecimal(orderParam.get("platformDiscountAmount").toString()));
		tradeOrdersDMO
		...
		tradeOrdersDMO.setIsOutDistribtion(Integer.valueOf(orderParam.get("isOutDistribtion").toString()));
		if (null != orderParam.get("hasProductplusFlag")) {
			...
			OtherCenterResDTO<Long> memberInfo = memberCenterRAO.getMemberIdByCode(orderCreateInfoReqDTO.getBuyerCode(), messageId);
			if(ResultCodeEnum.SUCCESS.getCode().equals(memberInfo.getOtherCenterResponseCode())){
				Long memberId = memberInfo.getOtherCenterResult();
				OtherCenterResDTO<MemberBuyerGradeInfoDTO> memberBuyerGradeInfo = memberCenterRAO.queryBuyerGradeInfo(memberId, messageId);
				if(ResultCodeEnum.SUCCESS.getCode().equals(memberBuyerGradeInfo.getOtherCenterResponseCode())){
					MemberBuyerGradeInfoDTO memberBuyerGradeInfoRes = memberBuyerGradeInfo.getOtherCenterResult();
					if(null != memberBuyerGradeInfoRes && null != memberBuyerGradeInfoRes.getIsVip()){
						tradeOrdersDMO.setIsVipMember(memberBuyerGradeInfoRes.getIsVip().intValue());
					}
				}
			}
		}
		...
		if(OrderStatusEnum.ORDER_DELIVERY_TYPE_SINCE.getCode().equals(tradeOrdersDMO.getDeliveryType())){
		   LOGGER.info("页面传入的是自提,则查询会员中心，将注册地址插入订单表开始messageId:"+messageId);
		   OtherCenterResDTO<Long> memberInfo = memberCenterRAO.getMemberIdByCode(orderCreateInfoReqDTO.getBuyerCode(), messageId);
		   Long memberId = memberInfo.getOtherCenterResult();
		   OtherCenterResDTO<MemberDetailInfo> memberDetailInfo = memberCenterRAO.getMemberDetailById(memberId, messageId);
		...
		if(OrderStatusEnum.OUT_DISTRIBTION.getCode().equals(tradeOrdersDMO.getIsOutDistribtion()==null?"":tradeOrdersDMO.getIsOutDistribtion().toString())){
		...
		tradeOrdersDMO.setHasUsedCoupon(orderTemp.isHasUsedCoupon()?Integer.valueOf(OrderStatusEnum.IS_SKILL_ORDER.getCode()):Integer.valueOf(OrderStatusEnum.NOT_SKILL_ORDER.getCode()));
		tradeOrdersDMO.setIsTimelimitedOrder(orderTemp.isTimelimitedOrder()?Integer.valueOf(OrderStatusEnum.USE_COUPON.getCode()):Integer.valueOf(OrderStatusEnum.NOT_USE_COUPON.getCode()));
		...
		LOGGER.info("MessageId:{} 创建订单--插入订单和订单状态履历表 耗时:{}", orderCreateInfoReqDTO.getMessageId(), endTime - startTime);
	...
	private OrderCreateInfoDMO insertTradeOrderItems(OrderCreateInfoReqDTO orderCreateInfoReqDTO,
		...
		if(null != commissionRatio){			
			tradeOrderItemsDMO.setCommissionAmount(new BigDecimal(df1.format(commissionRatio.multiply(tradeOrderItemsDMO.getGoodsAmount()))));
		}
		...
		String categoryIdList = (orderItemTemp.getFirstCategoryId()==null?"":orderItemTemp.getFirstCategoryId().toString())
				+","+(orderItemTemp.getSecondCategoryId()==null?"":orderItemTemp.getSecondCategoryId().toString())
				+","+(orderItemTemp.getThirdCategoryId()==null?"":orderItemTemp.getThirdCategoryId().toString());
		String categoryNameList = (orderItemTemp.getFirstCategoryName()==null?"":orderItemTemp.getFirstCategoryName())
				+","+(orderItemTemp.getSecondCategoryName()==null?"":orderItemTemp.getSecondCategoryName())
			    +","+(orderItemTemp.getThirdCategoryName()==null?"":orderItemTemp.getThirdCategoryName());
		...
		tradeOrderItemsDMO.setVipItemType(orderItemTemp.getVipItemType()==null?"":orderItemTemp.getVipItemType().toString());
		...
		tradeOrderItemsDMO.setGoodsFreight(orderItemTemp.getGoodsFreight()==null?new BigDecimal(0):orderItemTemp.getGoodsFreight());// 运费金额
		...
		BigDecimal goodsAmt = tradeOrderItemsDMO.getGoodsAmount() == null ? new BigDecimal(0):tradeOrderItemsDMO.getGoodsAmount();
		BigDecimal goodsFreight = tradeOrderItemsDMO.getGoodsFreight() == null ? new BigDecimal(0): tradeOrderItemsDMO.getGoodsFreight();
		BigDecimal orderItemTotalAmount = goodsAmt
				.add(goodsFreight).subtract(totalDiscountAmount);
		...
		tradeOrderItemsDMO.setGoodsRealPrice((goodsAmt.subtract(
				totalDiscountAmount)).divide(new BigDecimal(orderItemTemp.getGoodsCount()), 4, BigDecimal.ROUND_HALF_EVEN));
		...
		if(null != orderItem.getExtendMap() && StringUtilHelper.isNotNull(tradeOrderItemsDMO.getChannelCode())
			&& tradeOrderItemsDMO.getChannelCode().startsWith(GoodCenterEnum.EXTERNAL_CHANNELS.getCode())){
			Map<String,Object> extendMap = orderItem.getExtendMap();
			String outerChannelOrderNo = extendMap.get("outerChannelOrderNo")==null?"":extendMap.get("outerChannelOrderNo").toString();
			String outerChannelPuchaseStatus = extendMap.get("outerChannelPuchaseStatus")==null?"":extendMap.get("outerChannelPuchaseStatus").toString();
		...
		if(null != extendMap){
			tradeOrderItemsDMO.setSkuErpCode(extendMap.get("skuErpCode")==null?"":extendMap.get("skuErpCode").toString());
			tradeOrderItemsDMO.setSkuEanCode(extendMap.get("eanCode")==null?"":extendMap.get("eanCode").toString());
			tradeOrderItemsDMO.setErpFiveCategoryCode(extendMap.get("erpFiveCategoryCode")==null?"":extendMap.get("erpFiveCategoryCode").toString());
		}
		...
		if (OrderStatusEnum.OUT_DISTRIBTION.getCode()
				.equals(orderItemTemp.getIsOutDistribtion()==null?"":orderItemTemp.getIsOutDistribtion().toString())) {
		...
		Long endTime = System.currentTimeMillis();
		LOGGER.info("MessageId:{} 创建订单--更新插入订单行 耗时:{}", orderCreateInfoReqDTO.getMessageId(), endTime - startTime);
	...
	private void insertTradeOrderItemsDiscount(OrderCreateInfoReqDTO orderCreateInfoReqDTO,
		...
		List<OrderItemCouponDTO> avalibleCouponList = orderItemTemp.getAvalibleCouponList();
		LOGGER.info("从促销中心返回优惠券信息avalibleCouponList:"+JSONObject.toJSONString(avalibleCouponList));
		String levelCode = "";
		String buyerCouponCode = "";
		String couponType = "";
		if (OrderStatusEnum.PROMOTION_TYPE_COUPON.getCode().equals(orderCreateInfoReqDTO.getPromotionType())) {
			if(null != avalibleCouponList && avalibleCouponList.size()>0){
			...
			// 目前只有平台优惠
			tradeOrderItemsDiscountDMO.setCouponProviderType(OrderStatusEnum.COUPON_PROVIDER_TYPE_PLATFORM.getCode());
		...
		String messageId = orderCreateInfoReqDTO.getMessageId();
		LOGGER.info("从促销中心查询出优惠券信息MessageId:"+messageId+"levelCode:"+levelCode+" buyerCouponCode"+buyerCouponCode+" couponType"+couponType);
		Long startTime = System.currentTimeMillis();
		int update = tradeOrderItemsDiscountDAO.updateTradeOrderItemsDiscount(tradeOrderItemsDiscountDMO);
		if(update == ZERO){			
