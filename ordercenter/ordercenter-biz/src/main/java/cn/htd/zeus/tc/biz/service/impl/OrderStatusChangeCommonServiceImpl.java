...
public class OrderStatusChangeCommonServiceImpl implements OrderStatusChangeCommonService {
	 ...
	 * 
	 * @param isOrderError
	 * 是不是有异常订单,如果为true就更新订单和订单行的errorStatus，如果false就将orderErrorStatus、
	 * orderErrorReason更新成空 有异常订单的时候不需要更新订单状态
	 * 
	 * 如果只更新异常订单 directUpdateOrder:true isOrderError:true status:""
	 * statusText:"" orderErrorStatus:有值 orderErrorReason:有值 如果只更新订单状态
	 * directUpdateOrder:true|false isOrderError:false status:有值 statusText：有值
	 * orderErrorStatus:"" orderErrorReason:""
	 */
	@Override
	public void orderStatusChange(String orderNo, String orderItems, String status,
		...
		if (null != orderItemNoArray && orderItemNoArray.length > 0) {
			for (int i = 0; i < orderItemNoArray.length; i++) {
				...
				if (isOrderError) {
					...
					tradeOrderItemStatusHistoryService
							.insertTradeOrderItemStatusHistory(recordItem);
				}
		...
		if (directUpdateOrder) {
			updateOrder(status, orderNo, statusText, orderErrorStatus, orderErrorReason,
					isOrderError);
		} else {
	...
	public void updateOrderErrorVMSPayStatus(TradeOrdersDMO tradeOrdersDMO) {
		if (null != tradeOrdersDMO && StringUtilHelper.isNotNull(tradeOrdersDMO.getOrderNo())) {
			List<TradeOrderItemsDMO> itemDmo = tradeOrderItemsDAO
					.selectOrderItemsByOrderNo(tradeOrdersDMO.getOrderNo());
			...
			for (TradeOrderItemsDMO tempItemDmo : itemDmo) {
				if (StringUtilHelper.isNotNull(tempItemDmo.getOrderItemStatus())
					...
					if (orderItemStatus.equals(OrderStatusEnum.PAYED_POST_STRIKEA_DOWNING.getCode())
							|| orderItemStatus
									.equals(OrderStatusEnum.PAYED_POST_STRIKEA_SUCCESS_PRE_OPEN_LIST
											.getCode())) {
					...
					if (StringUtilHelper.isNotNull(orderItemErrorStatus)) {
						itemErrorStatusMap.put(orderItemErrorStatus,
								tempItemDmo.getOrderItemErrorReason());
					}
			...
			if (minItemStatus == 31) {
				if (StringUtilHelper.isNotNull(
						itemStatusMap.get(OrderStatusEnum.PAYED_POST_STRIKEA_DOWNING.getCode()))) {
					minItemStatus = Integer.valueOf(itemStatusMap
							.get(OrderStatusEnum.PAYED_POST_STRIKEA_DOWNING.getCode()));
				} else if (StringUtilHelper.isNotNull(itemStatusMap
						.get(OrderStatusEnum.PAYED_POST_STRIKEA_SUCCESS_PRE_OPEN_LIST.getCode()))) {
					minItemStatus = Integer.valueOf(itemStatusMap.get(
			...
			if (!itemErrorStatusMap.isEmpty()) {
				Entry<String, String> entry = (Entry<String, String>) itemErrorStatusMap.entrySet()
						.iterator().next();
