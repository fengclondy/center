 ...
 * 定时任务执行-京东抛单接口
 */
public class JDCreateOrderTask implements IScheduleTaskDealMulti<JDOrderInfoDMO>{
	
	private static final Logger LOGGER = LoggerFactory.getLogger(JDCreateOrderTask.class);
	...
	public List<JDOrderInfoDMO> selectTasks(String taskParameter, String ownSign, int taskQueueNum,
		...
		LOGGER.info("【selectTasks】【京东确认预占库存订单(京东确认订单)接口-开始执行】【请求参数：】【taskParameter:{},ownSign:{},taskQueueNum:{},taskItemList:{},eachFetchDataNum:{}】",
		...
		if(null != taskParameterMap && StringUtilHelper.isNotNull(taskParameterMap.get("jdSendCount"))){
			paramMap.put("jdSendCount", Integer.valueOf(taskParameterMap.get("jdSendCount").toString()).intValue());
		}else{
			paramMap.put("jdSendCount", zero);
		}
		...
		try{
			if (taskItemList != null && taskItemList.size() > 0) {
		        ...
		        jdOrderInfoList = jdCreateOrderService.selectJDOrderNOFromJDOrderInfo(paramMap);
			}
		}catch(Exception e){
	...
	public boolean execute(JDOrderInfoDMO[] tasks, String ownSign) throws Exception {
		LOGGER.info("【execute】【京东确认预占库存订单(京东确认订单)接口-开始执行】【请求参数：】【tasks:{},ownSign:{}】",new Object[]{JSONObject.toJSONString(tasks),JSONObject.toJSONString(ownSign)});
		boolean result = true;
		try {
			if (tasks != null && tasks.length > 0) {
				jdCreateOrderService.jdSureCreateOrder(tasks);
			}
		}catch (Exception e) {
			result = false;
			StringWriter w = new StringWriter();
