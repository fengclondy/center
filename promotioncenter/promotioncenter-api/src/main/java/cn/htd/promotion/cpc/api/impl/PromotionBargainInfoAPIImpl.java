...
public class PromotionBargainInfoAPIImpl implements PromotionBargainInfoAPI {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(PromotionBargainInfoAPIImpl.class);
	...
	public ExecuteResult<PromotionBargainInfoResDTO> getPromotionBargainInfoDetail(
		...
		try {
			...
			PromotionBargainInfoResDTO promotionBargainInfo = promotionBargainInfoService
			...
			LOGGER.error(
					MessageFormat
							.format("MessageId:{0} 调用方法promotionBargainInfoService.getPromotionBargainInfoDetail出现异常{1}",
									buyerBargainLaunch.getMessageId(),
									JSON.toJSONString(buyerBargainLaunch)), e);
		}
		return result;
	}
	...
	public ExecuteResult<DataGrid<PromotonInfoResDTO>> queryPromotionInfoListBySellerCode(
		...
		if (null != reqDTO) {
			return promotionBargainInfoService
	...
	public ExecuteResult<DataGrid<PromotionBargainOverviewResDTO>> queryPromotionBargainOverview(
		...
		if (!StringUtils.isEmpty(promotionId)) {
			return promotionBargainInfoService.queryPromotionBargainOverview(
					promotionId, page);
		} else {
			result.setCode(ResultCodeEnum.PROMOTION_PARAM_IS_NULL.getCode());
			result.setErrorMessage(ResultCodeEnum.PROMOTION_PARAM_IS_NULL
	...
	public ExecuteResult<PromotionExtendInfoDTO> addPromotionBargainInfo(
		...
		if (null != promotionExtendInfoDTO) {
			return promotionBargainInfoService
					.addPromotionBargainInfo(promotionExtendInfoDTO);
		} else {
			result.setCode(ResultCodeEnum.PROMOTION_PARAM_IS_NULL.getCode());
			result.setErrorMessage(ResultCodeEnum.PROMOTION_PARAM_IS_NULL
					.getMsg());
	...
	public ExecuteResult<PromotionExtendInfoDTO> updateBargainInfo(
		...
		if (null != promotionExtendInfoDTO) {
			return promotionBargainInfoService
					.updateBargainInfo(promotionExtendInfoDTO);
		} else {
			result.setCode(ResultCodeEnum.PROMOTION_PARAM_IS_NULL.getCode());
			result.setErrorMessage(ResultCodeEnum.PROMOTION_PARAM_IS_NULL
					.getMsg());
