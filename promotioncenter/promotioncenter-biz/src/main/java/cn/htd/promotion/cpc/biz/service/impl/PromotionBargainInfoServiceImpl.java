    ...
    public PromotionBargainInfoResDTO getPromotionBargainInfoDetail(BuyerBargainLaunchReqDTO buyerBargainLaunch) {
      ...
      //查询活动详情
        ...
        if(promotionBargainInfo1 != null){//曾经发起过砍价
        	promotionBargainInfo.setBargainCodeD(promotionBargainInfo1.getBargainCodeD());
        	promotionBargainInfo.setLaunchTime(promotionBargainInfo1.getLaunchTime());
        	promotionBargainInfo.setBargainOverTime(promotionBargainInfo1.getBargainOverTime());
        	promotionBargainInfo.setIsBargainOver(promotionBargainInfo1.getIsBargainOver());
        	promotionBargainInfo.setGoodsCurrentPrice(promotionBargainInfo1.getGoodsCurrentPrice());
        }else{
        	promotionBargainInfo.setGoodsCurrentPrice(promotionBargainInfo.getGoodsCostPrice());
        }
        //根据砍价编码查询砍价记录
		...
		try {
			...
			if(null != promotionInfoList && !promotionInfoList.isEmpty()){
				for (PromotionInfoDTO promotionInfo : promotionInfoList) {
					...
					resDTO.setHasUpFlag(promotionInfo.getHasUpFlag() == null ? 0 : promotionInfo.getHasUpFlag().intValue());
