...
public class PromotionLotteryCommonServiceImpl implements
		PromotionLotteryCommonService {

	private static final Logger logger = LoggerFactory
			.getLogger(PromotionLotteryCommonServiceImpl.class);
	 ...
	 * @param promotionId
	 */
	public PromotionExtendInfoDTO getRedisLotteryInfo(String promotionId,
		...
		validStatus = promotionRedisDB.getHash(RedisConst.REDIS_LOTTERY_VALID,
				promotionId);
		if (!StringUtils.isEmpty(validStatus)
				&& !dictMap
						.get(DictionaryConst.TYPE_PROMOTION_VERIFY_STATUS
								+ "&"
								+ DictionaryConst.OPT_PROMOTION_VERIFY_STATUS_VALID)
						.equals(validStatus)) {
			throw new PromotionCenterBusinessException(
					ResultCodeEnum.PROMOTION_NOT_VALID.getCode(), "抽奖活动ID:"
							+ promotionId + " 该活动未上架");
		}
		lotteryJsonStr = promotionRedisDB.getHash(
				RedisConst.REDIS_LOTTERY_INFO, promotionId);
		lotteryInfo = JSON.parseObject(lotteryJsonStr,
				PromotionExtendInfoDTO.class);
		if (lotteryInfo == null) {
			throw new PromotionCenterBusinessException(
					ResultCodeEnum.PROMOTION_NOT_EXIST.getCode(), "抽奖活动ID:"
							+ promotionId + " 该活动不存在");
		}
		return lotteryInfo;
	}
	 ...
	 * @throws PromotionCenterBusinessException
	 */
	public boolean checkBuyerPromotionLotteryValid(
			PromotionExtendInfoDTO promotionInfoDTO,
			DrawLotteryReqDTO requestDTO, Map<String, String> dictMap)
			throws PromotionCenterBusinessException {
		...
		if (nowDt.before(promotionInfoDTO.getEffectiveTime())) {
			throw new PromotionCenterBusinessException(
					ResultCodeEnum.PROMOTION_NO_START.getCode(), "抽奖活动编号:"
							+ promotionId + " 该活动未开始");
		}
		if (nowDt.after(promotionInfoDTO.getInvalidTime())) {
			throw new PromotionCenterBusinessException(
					ResultCodeEnum.PROMOTION_HAS_EXPIRED.getCode(), "抽奖活动编号:"
							+ promotionId + " 该活动已结束");
		}
		if (nowDt.before(DateUtil.getNowDateSpecifiedTime(promotionInfoDTO
				.getEachStartTime()))) {
			throw new PromotionCenterBusinessException(
					ResultCodeEnum.LOTTERY_NOT_IN_START_TIME.getCode(),
					"抽奖活动编号:"
							+ promotionId
							+ " 时间段:"
							+ DateUtil.format(promotionInfoDTO
									.getEachStartTime())
							+ "~"
							+ DateUtil.format(promotionInfoDTO.getEachEndTime())
							+ " 当前时间未到该活动抽奖开始时间");
		}
		if (nowDt.after(DateUtil.getNowDateSpecifiedTime(promotionInfoDTO
				.getEachEndTime()))) {
			throw new PromotionCenterBusinessException(
					ResultCodeEnum.LOTTERY_HAS_PASSED_END_TIME.getCode(),
					"抽奖活动编号:"
							+ promotionId
							+ " 时间段:"
							+ DateUtil.format(promotionInfoDTO
		...
		if (!baseService.checkPromotionBuyerRule(promotionInfoDTO,
				buyerCheckInfo, dictMap)) {
			throw new PromotionCenterBusinessException(
					ResultCodeEnum.LOTTERY_BUYER_NO_AUTHIORITY.getCode(),
					"该活动粉丝没有抽奖权限 入参:" + JSON.toJSONString(requestDTO));
		}
		lotteryTimesInfoMap = promotionRedisDB
				.getHashOperations(RedisConst.REDIS_LOTTERY_TIMES_INFO + "_"
						+ promotionId);
		...
		if (!baseService.checkPromotionSellerRule(promotionInfoDTO,
				requestDTO.getSellerCode(), dictMap)) {
			throw new PromotionCenterBusinessException(
					ResultCodeEnum.LOTTERY_SELLER_NO_AUTHIORITY.getCode(),
					"会员店没有参加本次抽奖活动 入参:" + JSON.toJSONString(requestDTO));
		}
		buyerTimesInfoMap = promotionRedisDB
				.getHashOperations(RedisConst.REDIS_LOTTERY_BUYER_TIMES_INFO
		...
		if (Integer.parseInt(buyerTimesInfoMap
				.get(RedisConst.REDIS_LOTTERY_BUYER_PARTAKE_TIMES)) <= 0) {
			if (!lotteryTimesInfoMap
					.containsKey(RedisConst.REDIS_LOTTERY_BUYER_TOP_EXTRA_PARTAKE_TIMES)
					|| Integer
							.parseInt(lotteryTimesInfoMap
									.get(RedisConst.REDIS_LOTTERY_BUYER_TOP_EXTRA_PARTAKE_TIMES)) <= 0) {
				throw new PromotionCenterBusinessException(
						ResultCodeEnum.LOTTERY_BUYER_NO_MORE_DRAW_CHANCE
								.getCode(),
						"粉丝已经用完了所有抽奖机会，需分享获得额外抽奖机 入参:"
								+ JSON.toJSONString(requestDTO));
			} else if (!lotteryTimesInfoMap
					.containsKey(RedisConst.REDIS_LOTTERY_BUYER_SHARE_EXTRA_PARTAKE_TIMES)
					|| Integer
							.parseInt(lotteryTimesInfoMap
									.get(RedisConst.REDIS_LOTTERY_BUYER_SHARE_EXTRA_PARTAKE_TIMES)) <= 0
					|| (buyerTimesInfoMap
							.containsKey(RedisConst.REDIS_LOTTERY_BUYER_HAS_TOP_EXTRA_TIMES) && YesNoEnum.YES
							.getValue() == Integer
							.parseInt(buyerTimesInfoMap
									.get(RedisConst.REDIS_LOTTERY_BUYER_HAS_TOP_EXTRA_TIMES)))) {
				throw new PromotionCenterBusinessException(
						ResultCodeEnum.LOTTERY_BUYER_NO_MORE_EXTRA_CHANCE
								.getCode(),
						"粉丝已经用完了自有和分享额外获取的抽奖机 入参:"
								+ JSON.toJSONString(requestDTO));
			}
		}
		return true;
	 ...
	 * @throws PromotionCenterBusinessException
	 */
	public boolean checkPromotionLotteryValid(
			PromotionExtendInfoDTO promotionInfoDTO,
			DrawLotteryReqDTO requestDTO, Map<String, String> dictMap)
			throws PromotionCenterBusinessException {
		String promotionId = requestDTO.getPromotionId();
	...
	@Override
	public boolean checkScratchCardValid(
			PromotionExtendInfoDTO promotionInfoDTO,
		...
		Map<String, String> buyerTimesInfoMap = promotionRedisDB
				.getHashOperations(RedisConst.REDIS_LOTTERY_BUYER_TIMES_INFO
						+ "_" + promotionId + "_" + buyerCode);
		String buyerPartakeTimes = buyerTimesInfoMap
				.get(RedisConst.REDIS_LOTTERY_BUYER_PARTAKE_TIMES);
		if (!StringUtils.isEmpty(buyerPartakeTimes)
				&& Integer.parseInt(buyerPartakeTimes) <= 0) {
			throw new PromotionCenterBusinessException(
					ResultCodeEnum.LOTTERY_BUYER_CURRENT_DAY_NO_MORE_DRAW_CHANCE
	 ...
	 * @param ticket
	 */
	public void doDrawLottery(DrawLotteryReqDTO requestDTO,
			BuyerWinningRecordDTO errorWinningRecord, String ticket) {
		String promotionId = requestDTO.getPromotionId();
		String buyerCode = requestDTO.getBuyerCode();
		String sellerCode = requestDTO.getSellerCode();
		String thanksType = dictionary.getValueByCode(
				DictionaryConst.TYPE_PROMOTION_REWARD_TYPE,
		...
		try {
			awardPercentStr = promotionRedisDB.getHash(
			...
			accuList = JSON.parseArray(awardPercentStr,
					PromotionAccumulatyDTO.class);
			...
			if (winningRecordDTO.getRewardType().equals(
					errorWinningRecord.getRewardType())
					|| thanksType.equals(winningRecordDTO.getRewardType())) {
				promotionRedisDB.incrHash(
						RedisConst.REDIS_LOTTERY_BUYER_TIMES_INFO + "_"
								+ promotionId + "_" + buyerCode,
						RedisConst.REDIS_LOTTERY_BUYER_WINNING_TIMES);
				promotionRedisDB
						.incr(RedisConst.REDIS_LOTTERY_SELLER_WINED_TIMES + "_"
								+ promotionId + "_" + sellerCode);
			}
		...
		promotionRedisDB
				.setHash(RedisConst.REDIS_LOTTERY_BUYER_AWARD_INFO, promotionId
						+ "_" + sellerCode + "_" + buyerCode + "_" + ticket,
						JSON.toJSONString(winningRecordDTO));
	}

	private BuyerWinningRecordDTO drawLotteryAward(
			DrawLotteryReqDTO requestDTO, List<PromotionAccumulatyDTO> accuList)
			throws PromotionCenterBusinessException {
		String promotionId = requestDTO.getPromotionId();
		String buyerCode = requestDTO.getBuyerCode();
		String sellerCode = requestDTO.getSellerCode();
		...
		if (promotionRedisDB.decrHash(
				RedisConst.REDIS_LOTTERY_BUYER_TIMES_INFO + "_" + promotionId
						+ "_" + buyerCode,
				RedisConst.REDIS_LOTTERY_BUYER_PARTAKE_TIMES).longValue() < 0) {
			promotionRedisDB.incrHash(RedisConst.REDIS_LOTTERY_BUYER_TIMES_INFO
					+ "_" + promotionId + "_" + buyerCode,
					RedisConst.REDIS_LOTTERY_BUYER_PARTAKE_TIMES);
			throw new PromotionCenterBusinessException(
					ResultCodeEnum.LOTTERY_BUYER_NO_MORE_DRAW_CHANCE.getCode(),
					"抽奖活动编号:" + promotionId + " 会员店:" + sellerCode + " 抽奖粉丝编号:"
							+ buyerCode + " 粉丝已经用完了所有抽奖机会");
		}
		if (promotionRedisDB.decrHash(
				RedisConst.REDIS_LOTTERY_BUYER_TIMES_INFO + "_" + promotionId
						+ "_" + buyerCode,
			...
			throw new PromotionCenterBusinessException(
					...
					"抽奖活动编号:" + promotionId + " 会员店:" + sellerCode + " 抽奖粉丝编号:"
							+ buyerCode + " 粉丝已达中奖次数上限");
		}
		if (promotionRedisDB.decr(
				RedisConst.REDIS_LOTTERY_SELLER_WINED_TIMES + "_" + promotionId
						+ "_" + sellerCode).longValue() < 0) {
			throw new PromotionCenterBusinessException(
					ResultCodeEnum.LOTTERY_SELLER_REACH_WINNING_LIMMIT
							.getCode(),
					"抽奖活动编号:" + promotionId + " 会员店:" + sellerCode + " 抽奖粉丝编号:"
							+ buyerCode + " 会员店已达中奖次数上限");
		}
		targetAccuList = new ArrayList<PromotionAccumulatyDTO>();
		for (PromotionAccumulatyDTO checkAccuDTO : accuList) {
			lotteryKey = RedisConst.REDIS_LOTTERY_AWARD_PREFIX + promotionId
					+ "_" + checkAccuDTO.getLevelCode();
			if ("0".equals(checkAccuDTO.getLevelAmount())
					|| promotionRedisDB.getLlen(lotteryKey) <= 0) {
				continue;
			}
		...
		if (targetAccuList.isEmpty()) {
			...
			for (int i = 0; i < targetAccuList.size(); i++) {
				...
				if (lastAccuDTO == null) {
					...
					currentPercent = Integer.parseInt(lastAccuDTO
							.getLevelAmount());
				}
				currentPercent += Integer.parseInt(tmpAccuDTO.getLevelAmount())
						* 100 / totalPercent;
	 ...
	 */
	@Override
	public void initPromotionLotteryRedisInfoWithThread(
			final PromotionExtendInfoDTO promotionInfoDTO) {
		new Thread() {
			public void run() {
	 ...
	 */
	@Override
	public void initPromotionLotteryRedisInfo(
			final PromotionExtendInfoDTO promotionInfoDTO) {
		...
		try {
			...
			stringRedisTemplate
					.executePipelined(new RedisCallback<List<Object>>() {
						@Override
						public List<Object> doInRedis(RedisConnection connection)
								throws DataAccessException {
							StringRedisConnection stringRedisConnection = (StringRedisConnection) connection;
							String promotionId = promotionInfoDTO
									.getPromotionId();
							List<PromotionAwardInfoDTO> promotionAwardInfoDTOList = (List<PromotionAwardInfoDTO>) promotionInfoDTO
							...
							String redisKey = "";
							long pushCnt = 0L;
							long diffTime = promotionInfoDTO.getInvalidTime()
									.getTime() - new Date().getTime();
							int seconds = (int) (diffTime / 1000);
							try {
								timesInfoMap
										.put(RedisConst.REDIS_LOTTERY_BUYER_DAILY_DRAW_TIMES,
												String.valueOf(promotionInfoDTO
														.getDailyBuyerPartakeTimes()));
								timesInfoMap
										.put(RedisConst.REDIS_LOTTERY_BUYER_DAILY_WINNING_TIMES,
												String.valueOf(promotionInfoDTO
														.getDailyBuyerWinningTimes()));
								timesInfoMap
										.put(RedisConst.REDIS_LOTTERY_SELLER_DAILY_TOTAL_TIMES,
												String.valueOf(promotionInfoDTO
														.getDailyWinningTimes()));
								if (YesNoEnum.YES.getValue() == promotionInfoDTO
										.getIsShareTimesLimit().intValue()) {
									timesInfoMap
											.put(RedisConst.REDIS_LOTTERY_BUYER_SHARE_EXTRA_PARTAKE_TIMES,
													String.valueOf(promotionInfoDTO
															.getShareExtraPartakeTimes()));
									if (promotionInfoDTO
											.getTopExtraPartakeTimes() != null
											&& promotionInfoDTO
													.getTopExtraPartakeTimes()
													.intValue() >= 0) {
										timesInfoMap
												.put(RedisConst.REDIS_LOTTERY_BUYER_TOP_EXTRA_PARTAKE_TIMES,
														String.valueOf(promotionInfoDTO
																.getTopExtraPartakeTimes()));
									}
								}
								...
								for (PromotionAwardInfoDTO awardInfoDTO : promotionAwardInfoDTOList) {
									...
									redisKey = RedisConst.REDIS_LOTTERY_AWARD_PREFIX
											+ promotionId + "_" + levelCode;
									stringRedisConnection.del(redisKey);
									pushCnt = awardInfoDTO.getProvideCount()
											.longValue();
									totalAwardCnt += awardInfoDTO
											.getProvideCount().longValue();
									if (pushCnt > 0) {
										buyerWinningRecordDTO = new BuyerWinningRecordDTO();
										buyerWinningRecordDTO
												.setBuyerWinningRecordByAwardInfo(awardInfoDTO);
										buyerWinningRecordDTO
												.setBuyerWinningRecordByPromoitonInfo(promotionInfoDTO);
										while (pushCnt > 0) {
											stringRedisConnection.rPush(
													redisKey,
													JSON.toJSONString(buyerWinningRecordDTO));
											pushCnt--;
										}
									}
									stringRedisConnection.expire(redisKey,
											seconds);
								}
								timesInfoMap
										.put(RedisConst.REDIS_LOTTERY_AWARD_WINNING_PERCENTAGE,
												JSON.toJSONString(accuList));
								timesInfoMap
										.put(RedisConst.REDIS_LOTTERY_AWARD_TOTAL_COUNT,
												String.valueOf(totalAwardCnt));
								stringRedisConnection
										.del(RedisConst.REDIS_LOTTERY_TIMES_INFO
												+ "_" + promotionId);
								stringRedisConnection.hMSet(
										RedisConst.REDIS_LOTTERY_TIMES_INFO
												+ "_" + promotionId,
										timesInfoMap);
								stringRedisConnection.expire(
										RedisConst.REDIS_LOTTERY_TIMES_INFO
												+ "_" + promotionId, seconds);
								stringRedisConnection.hSet(
										RedisConst.REDIS_LOTTERY_INFO,
										promotionId,
										JSON.toJSONString(promotionInfoDTO));
								stringRedisConnection.hSet(
										RedisConst.REDIS_LOTTERY_VALID,
										promotionId,
										promotionInfoDTO.getShowStatus());
								stringRedisConnection.hSet(
										RedisConst.REDIS_LOTTERY_INDEX,
										promotionInfoDTO.getPromotionType()
												+ "_" + promotionId,
										promotionInfoDTO.getEffectiveTime()
												.getTime()
												+ "_"
												+ promotionInfoDTO
														.getInvalidTime()
