...
public class NUtils {
	...
	public static void copyProperties(JSONObject source,Object target,boolean copyNull) throws Exception{
		if(source == null || target == null)
			return;
		...
		// 循环
		for(String key : keys){
			targetField = targetMap.get(key.toLowerCase());
			if(targetField == null){
				continue;
			}
			...
			if(!copyNull && isEmpty(source.get(key))){
				continue;
			}

			if("Integer".equals(targetField.getType().getSimpleName())){
				targetField.set(target,source.getInt(key));
			}else{
				targetField.set(target,source.get(key));
			}
